...
// The document's own section in the Changes tab, keyed on the header path:
// matching on body text would also hit AGENT_NOTES.md once a sent packet
// (which names the file) has been appended to it by the mock.
const docSection = (page: Page) => page.locator(".tc-file", { has: page.locator(".tc-fpath", { hasText: "docs/setup.md" }) });
...
test.beforeAll(async ({ request }) => {
  ...
  docPath = path.join(settled.worktree_path, "docs", "setup.md");
...
test("edit + comment + send as a patch arrive as one located packet", async ({ page }) => {
  ...
  await docSection(page).getByRole("button", { name: "Collaborate" }).click();
...
test("Write to file lands the edit in the worktree and tells the agent what changed", async ({ page, request }) => {
  ...
  await expect(docSection(page)).toContainText("The default port is 3000.");
});

...
test("edit + comment + send as a patch arrive as one located packet", async ({ page }) => {
  ...
  // no pencil), tagged, and not outdated, since the underlying file hasn't
  // changed since. Send cleared the edit draft too, so there is nothing to
  // restore or send: Send to agent is disabled.
  await collaborateOn(page, "docs/setup.md").click();

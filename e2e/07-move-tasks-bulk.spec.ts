// Multi-select and "Move to project…" for the task list, driving the bulk
// move gesture: tick, shift-click a range, move, and confirm the selection
// lands in the destination project's tray. Server rules (the skip report,
...
// tests/taskMoveBulk.test.ts.
//
// The last test covers a started task: a selection that includes a task that
// has run moves it only if that row's own checkbox is ticked, alongside what
// its worktree holds.
...
test.beforeAll(async ({ request }) => {
  ...
  // A real turn: the mock agent commits, so this task lands with a worktree,
  // a branch, and a commit main never took.
...
test("a shift-click with nothing selected yet still anchors the next range", async ({ page }) => {
  ...
  // No prior click, so this shift-click has no anchor and can only select
  // itself.
  await pickbox(page, TITLES[0]).click({ modifiers: ["Shift"] });
  await expect(page.locator(".pick-bar .pb-count")).toHaveText("1 selected");
  // It must become the anchor, or every later range gesture degrades to a
...
test("a multi-select moves every picked task to another project at once", async ({ page }) => {
  ...
  // All three arrive in the destination project.
...
test("a started task in the selection moves only if its own box is ticked", async ({ page, request }) => {
  ...
  // The checkout the answer was about is gone from disk and off the row.

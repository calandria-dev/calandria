...
// evidence you want when a spec fails: the transcript rows, the worktree the
// diff was read from, the branch a merge left behind. So a red run KEEPS it and
// prints where it is, and a green one leaves nothing — the reason the root
// exists in the first place is post-mortem, and a passing run has no post-mortem.
// Before this, every local `npm run test:e2e` and every `npx playwright test`
// left another full instance under the temp dir forever.
//
...
// they are the only hooks that see a stopped server AND the run's status (which
// `globalTeardown` is not given at all).
//
// The work is split across the two of them on purpose. `onEnd` is where the
// status arrives, but it is NOT reliably last: the list reporter prints its
// whole epilogue — the failure detail and the `1 failed` line — from its own
// `onEnd`, and being second in `reporter: []` did not put us after it
// (measured). `onExit` is called for every reporter only after every `onEnd`
// has resolved, so that is where the path gets printed and the tree removed —
// the line the developer needs lands under the failures it belongs to.
//
...
export default class CleanupReporter implements Reporter {
  ...
  async onExit(): Promise<void> {
    ...
    try {
      ...
      // lets an open file be unlinked and simply disappear, Windows refuses
      // with EBUSY/EPERM/ENOTEMPTY while any handle is open. The app's own
      // handles are gone — Playwright's webServer teardown has already killed
      // the `npm start` process tree — so what is left is the transient kind
      // (a Defender scan of a freshly written worktree, an indexer), which is
      // what Node's retry covers. A residual failure must not red a green
      // suite: the directory is under the temp dir, so it is reported.

// The push channel: every notification the server composes goes to every
// subscribed browser, through the same emitter the in-tab channel uses
// (lib/notifications/notify.ts calls pushNotification from its single exit),
// so the two stay consistent about what's worth a buzz and how it reads.
// Fire-and-forget: deliver() runs synchronously inside the runner's publish,
// inside the TURN that just stopped. The DB read is sync and cheap; everything
// after it is detached network I/O that must never be awaited by or throw
// into that turn. fetch-only, no SDK; pinned by tests/importGraph.test.ts.
...
/**
 * Deliver one encrypted message to one subscription. Every outcome, including
 * a network error, comes back as a result with status 0 instead of a thrown
 * error, so the fan-out above can prune and record without a try/catch per
 * device.
 */
...
// A failure is urgent enough to wake the device for; a question can wait for
// the next unlock. Either is still worth surfacing hours later: a stopped
// task stays stopped until someone picks it up, so the service holds the
// message a day for a phone that was off.
...
/** The deep link the worker opens on click, using the app's own ?project/?task keys. */
 ...
 * data cleared, permission revoked, or the service rotated it and the
 * pushsubscriptionchange re-post never arrived), and nothing will deliver
 * there again. Every other failure is recorded on the row for the device
 * list: a 401/403 usually means the VAPID key changed under the
 * subscription, which the browser fixes on its next visit by re-subscribing
 * (usePush.ts compares keys); a 5xx or timeout is a transient failure at the
 * push service.
 */

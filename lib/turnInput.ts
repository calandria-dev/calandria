...
// iterable held open (lib/agents/claude/driver.ts, the background-linger
...
// pending_messages.
//
// Tiny and SDK-free (pinned by tests/importGraph.test.ts): it is reached from
// the route layer, and lib/agents/registry.ts must never be dragged into that
// graph. It mirrors lib/abort.ts structurally: keyed by task id, kept on
// globalThis so dev HMR reloads don't orphan a live turn's handle, and
// identity-guarded on unregister so a finishing turn can never wipe a
...
// is in flight, the message opens a fresh one) or mid-thought (unsafe, which
...
export type TurnInputHandle = {
   ...
   * the live turn accepted it, in which case the caller owns persisting and
   * publishing it like any other user message. False means the turn can't
   * take it (mid-model-turn, closing, or already closed); queue it instead.
   *
 ...
 * in a state that can take a message, so it should be parked in the pending
 * queue instead.
 */

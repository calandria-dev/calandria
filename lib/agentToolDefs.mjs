...
export const SUGGEST_TASK = {
  ...
  params: {
    ...
    provider:
      "Which endpoint the task's turns run against. \"local\" pins it to the instance's local model server (Ollama / LM Studio) — " +
      "the way to DELEGATE routine, well-specified work (a rename, a test file, a mechanical migration) to a local model that costs no " +
      "cloud quota; write the brief for a smaller model, spelling out the files and the expected result. \"cloud\" pins it to the " +
      "agent's own login even in a project whose default is local. Omit to inherit the project's setting.",
    model:
      "The model to run the task on. With provider \"local\" this is the local server's model tag (e.g. qwen3-coder) and is required " +
      "unless the project already names one; with \"cloud\" an id from the agent's catalog. Omit to inherit.",
  },
...
export const UPDATE_TASK = {
  ...
  description:
    ...
    "The git base branch a task is built on is NOT a field here — changing it moves a real worktree, so it has its own tool, " +
    "`set_base_branch`.",
  params: {
    task:
    ...
    tags:
      ...
      "This sets MEMBERSHIP — to rename a tag, rewrite its brief or set the branch its tasks are based on, use `update_tag`.",
  },
  priorities: ["hi", "med", "lo"],
...
export const MOVE_TASK = {
  name: "move_task",
  description:
    "Move tasks from one project into another, keeping the tasks themselves — same ids, same descriptions, same history, same " +
    "sessions and spend. Use it when work was planned in one project and belongs in another (a repo added as its own project after " +
    "the plan was written), rather than re-filing with `suggest_task` and withdrawing the originals, which retypes every brief and " +
    "loses the ordering. " +
    "Move a whole dependency chain in ONE call: a `blocked_by` edge survives only when BOTH of its ends move together, and every " +
    "edge with one end left behind is dropped — you are told which, so you can decide whether to redraw it with `update_task`. " +
    "A task the user has already STARTED is refused. Its git worktree was cut from the old project's repo, so moving it means " +
    "destroying that checkout, and that is the user's answer to give — per task, from the board's Move dialog. A task with a turn " +
    "running in it is refused too, including the one you are calling from. " +
    "Refusals are per task: the movable ones still move, and the reply names the rest and why.",
  params: {
    tasks:
      "The ids to move, from `list_tasks`. Pass a dependency chain WHOLE — an edge between two tasks survives only if both of its " +
      "ends are in this list.",
    project:
      "Where to move them — a project id, or its exact name (case-insensitive), as returned by `list_projects`. An unrecognized " +
      "value is an error, never a fallback to this session's project.",
  },
};
...
export const SET_BASE_BRANCH = {
  ...
  description:
    "Change the git branch this task's worktree is based on — what it was cut from, what Sync catches it up to, and what " +
    "Merge lands it into. Use it when the work belongs on a feature or integration branch rather than the project's default. " +
    "The branch must already exist (locally or on the remote); it can't be another task's `calandria/…` branch, because that " +
    "branch is checked out in a live worktree. If this task has already made commits, nothing is rewritten — you are told how " +
    "far behind the new base you are, and syncing catches you up. To set the base for a whole plan at once, put it on the tag " +
    "with `update_tag` instead; tasks inherit it until their worktree is cut.",
  params: {
    branch:
      "The branch to base the task on, e.g. \"feature/auth\". It must already exist locally or on the remote — this tool " +
      "never creates one from thin air (a typo would mint a branch nobody meant). Pass \"\" to go back to inheriting the " +
      "default from the task's tags, or failing that from the project.",
    task:
      "Which task to retarget — its id, from `list_tasks`. Omit for the task this session is running in. Any other task in " +
      "the SAME project is fair game; the one refusal is a task with a turn running in it right now, which would have its " +
      "worktree moved under a live session.",
  },
};
...
export const CREATE_PR = {
  name: "create_pr",
  description:
    "Push this task's work branch to GitHub and open a pull request against its base branch — the ONE way a session can " +
    "say in git that its work is finished. Offered only on projects that land by pull request; on a project that lands by " +
    "merging there is nothing to open and this tool is not present. Use it when the work is done, tested and committed-worthy: " +
    "it commits whatever is still uncommitted in the worktree (the same commit Merge would make), pushes, and runs " +
    "`gh pr create`. Calling it again after more work re-pushes and updates the SAME pull request rather than opening a " +
    "second one, so it doubles as \"update my PR\". You cannot merge it and there is no tool that can: opening a PR is " +
    "proposing, merging is the user's decision. Reach for this rather than shelling out: a session's own `git push` and " +
    "`gh pr create` are usually refused, which is why the server does it for you.",
  params: {
    title:
      "Title for the pull request. Omit to use the task's own title. If the repository requires Conventional Commit titles, " +
      "write one (e.g. \"feat: add the create_pr tool\") — the task's title is prose and usually is not one.",
    body:
      "Body for the pull request, as Markdown. Omit to have it composed from the task's description and the latest session " +
      "summary. Pass one when you can describe what you actually changed better than the brief can — what changed, why, and " +
      "how it was verified.",
  },
};
...
export const UPDATE_TAG = {
  name: "update_tag",
  description:
    "Edit a tag as the plan changes — rename it, rewrite its brief, or set the git branch its tasks are based on. " +
    "Setting `base_branch` points every task carrying this tag from now on at that branch instead of the project's default; " +
    "members whose worktree has already been cut keep the branch they forked from, so this is safe to set mid-plan. " +
    "There is no delete — that's the user's call. This edits the TAG ITSELF; to change which tags a task carries, use " +
    "`update_task`'s `tags`.",
  params: {
    tag:
      "Which tag to edit — its id, or its exact name, as returned by `list_tags`. Names are matched exactly, so read them " +
      "from there rather than guessing. An unrecognized value is an error, never a new tag.",
    name:
      "New name for the tag. Omit to leave it alone. Names are unique per project — a name another tag already has is " +
      "refused rather than merging the two.",
    description:
      "New brief for the plan — what it is for, read by every session carrying the tag. Replaces the existing text rather " +
      "than appending to it. Omit to leave it alone.",
    color:
      "New badge tint — one of Calandria's tag colors, as a hex string. Omit to leave it alone; pass \"\" for the neutral " +
      "badge. A value outside the palette is refused with the accepted list, so there is nothing to memorize here.",
    base_branch:
      "The git branch tasks carrying this tag are cut from, catch up to and merge into, e.g. \"feature/auth\". Applies to " +
      "members whose worktree hasn't been cut yet; a member already cut keeps the branch its work is built on (retarget " +
      "that one with `set_base_branch`). Pass \"\" to clear it, so members follow the project's default again.",
  },
};

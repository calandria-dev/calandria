...
const MODEL_PARAM =
  "Model id to run on. Checked against the named provider's models turned on in Settings → Models, " +
  "unless the provider is a CLI's own bundled login, which has no such list to check. Omit to " +
...
export const SUGGEST_TASK = {
  ...
  description:
    ...
    "spec) from this worktree. Refuses an unrecognized `project`.",
  params: {
...
export const UPDATE_RUNBOOK = {
  ...
  params: {
    ...
    model:
      ...
      "one). Omit to leave it; pass \"\" to clear back to the provider's default.",
  },
  priorities: ["hi", "med", "lo"],
};

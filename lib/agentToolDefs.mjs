...
export const SUGGEST_TASK = {
  ...
  description:
    ...
    "NAME THE PLAN: when the tasks are steps of one feature, migration or refactor, pass the same `group` to every one of them — " +
    "it is created on first use, the user gets one chip for the whole plan, and each member session is told which group it is a step of.",
  params: {
    ...
    group:
      "The group this task belongs to — a named container for a feature, migration or refactor that takes several tasks. " +
      "Pass a group id or a group NAME (from `list_groups`); a name that doesn't exist yet is CREATED in the project the task " +
      "is filed into, so you can file a whole plan under one name in one pass without a setup call. Names are exact and unique " +
      "per project, so reuse the existing spelling rather than a near-miss. Every member session is told which group it belongs " +
      "to, what the group is for, and which step of it it is — so group a plan whenever its tasks are steps of one thing.",
  },
...
export const LIST_TASKS = {
  ...
  params: {
    ...
    group:
      "Only list tasks in this group — its id, or its exact name, as returned by `list_groups`. " +
      "Omit for the whole board. Every row carries its own group either way. An unrecognized value is an error, never a fallback.",
  },
};
...
export const LIST_GROUPS = {
  name: "list_groups",
  description:
    "List a project's task GROUPS — the named containers a multi-task feature, migration or refactor is planned under. " +
    "Each one comes back with its description, how many of its members are done, running or waiting on the user, and the " +
    "members themselves (id, title, status). Read-only. This is the one call that answers \"how is the auth migration going\", " +
    "and the way to find the exact name of a group before passing it to `suggest_task`, `update_task` or `list_tasks` — names " +
    "are matched exactly, so read them from here rather than guessing.",
  params: {
    project:
      "Which project's groups to list — its id, or its exact name (case-insensitive), as returned by `list_projects`. " +
      "Omit for the project this session is running in. An unrecognized value is an error, never a fallback.",
  },
};
...
export const UPDATE_TASK = {
  ...
  params: {
    ...
    group:
      "Move this task into a group — its id, or the EXACT name of a group that already exists (from `list_groups`), in the task's " +
      "own project. Pass \"\" to take it out of its group. Unlike `suggest_task` this never creates one: a typo here would mint a " +
      "near-duplicate of a group the user is already using, so an unknown value refuses the whole call and nothing else in it lands.",
  },
  priorities: ["hi", "med", "lo"],

...
// a project" and "a turn is streaming" is identical, so it lives in one module
...
// back), which is the condition that makes Turbopack's async-ness propagation
// reliable. Routes should still reach it through `await import()`, as they
// already do for lib/scheduler.ts.
...
export interface DispatchInput {
  ...
  /**
   * Called with the new task id after createTask and before the launch. This
   * is the ledger seam: fireSchedule has to link the run to the task while the
 ...
 * launch, not the creation, is what fell over. It is a real, retryable task,
 * and callers that report the failure should be able to point at it.
 */
...
export async function dispatchPromptTask(input: DispatchInput): Promise<DispatchResult> {
  // ---- preflight: fail with something actionable instead of minting a task
  ...
  // Checked for this agent specifically; never falls back to another, which
  // would run the work on the wrong login without saying so.
  ...
  try {
    ...
    try {
      ...
      await withTaskLock(created.id, async () => {
        ...
        // fallback to repo_path. Throwing is different: a stale index.lock
        // from a crashed process, a disk-full git op, a detached HEAD, and
        // swallowing it here would launch straight into the user's real
        // checkout instead of an isolated worktree. So it isn't caught here;
        ...
        // the same mechanism both callers use for every other dispatch
        // failure: fireSchedule settles the run "failed" with this same
        // message, and the runbook route turns it into a visible 400.
    ...
    // with its "Repair worktree" button attached, one click away from a launch
    // since the task is unstarted.

/* Host-header router for public service hostnames, loaded by server.js (raw
 * Node, CommonJS) via dynamic import.
 *
 * Requests for <slug>--<appHost> are dispatched here before Next.js sees
 * them: look up the slug in the live service registry (lib/services.ts on
 * globalThis), enforce its visibility, then reverse-proxy to
 * 127.0.0.1:<port>, including WebSocket upgrades for Vite/Next HMR; anything
 * else falls through to Next. Auth per visibility: public is open, shared
 * accepts a tokened link or its gate cookie, private requires a gate cookie
 * minted via /api/services/grant on the app hostname.
 */
...
import { safeRedirectPath } from "./auth/local-origin.mjs";
 ...
 * keeps the default), but hostname routing also requires the explicit
 * CALANDRIA_SERVICE_HOSTS opt-in; enabling the feature alone exposes nothing. */
export function serviceRoutingEnabled() {
  const v = readEnv("CALANDRIA_FEATURE_SERVICES");
  return (v ? truthy(v) : true) && serviceHostsEnabled() && !!appHostFromEnv();
}

function registry() {
  return globalThis.__calandriaServices || null;
}
...
// The apex the fleet lives under (ishan.calandria.example.com -> calandria.example.com),
...
<style>
  ...
  <p>${esc(message)}</p>
  ${app ? `<a class="btn" href="${esc(app)}">Open Calandria</a>` : ""}
...
// an open redirect. Shared with the grant route so the two cannot drift; the
// backslash/whitespace subtleties are documented at the definition.
...
function authorize(req, res, m, url) {
  ...
  // /api/services/grant (private), then set the cookie and land on `next`.
  if (url.pathname === "/__calandria/auth") {
    ...
    if (secret && verifyGateToken(secret, t, m.slug)) {
      ...
      statusPage(res, 403, m.name, "This sign-in link is invalid or expired. Open the service again from Calandria.");
    }
    return { handled: true };
  }
...
export function handleServiceRequest(req, res) {
  ...
  if (!m) {
    statusPage(res, 404, name, "No service by this name is registered on this instance. Start it from Calandria's Services panel.");
    return true;
  }
  const url = new URL(req.url || "/", "http://x");
  if (!isLive(m)) {
    statusPage(res, 503, m.name, "The service is registered but not running right now. Start it from Calandria, then reload.");
    return true;
  }

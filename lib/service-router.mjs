...
export function serviceRoutingEnabled() {
  // Control-plane mode is a VETO, not a fourth requirement: a fleet's control
  // plane runs this same image off the same env template, so it can arrive with
  // ORCH_SERVICE_HOSTS=1 inherited, and it must never dispatch a user's service
  // off its own hostname. Hoisted out of the && chain below because it is the
  // one condition that silently drops services back to http://localhost:<port>
  // with everything else correctly configured — documented in .env.example.
  if (process.env.ORCH_CONTROL_PLANE === "1") return false;

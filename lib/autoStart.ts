...
//
// A blocked task never starts on its own unless the user set its
// `auto_start` flag ("Start when unblocked"). When a blocker reaches a
// TERMINAL status (done or cancelled, matching blocks() below), this module
// finds its auto-start dependents whose last unfinished blocker just
// cleared and launches each one's first turn, exactly as if the user had
// pressed "Start session", mirroring the initial-turn branch of
// POST /api/tasks/[id]/messages. The scheduling decision follows from the
// resulting state, not from which caller produced it (PATCH
// /api/tasks/[id] and the withdraw_suggestion agent tool both reach it), so
// the same row means the same thing regardless of path.
//
// lib/runner.ts is reached through a call-time `await import()`, never a
// static import: the runner pulls in both ESM-only agent SDKs, which
// Turbopack emits as async externals, so a static importer compiles sync
// and reads every runner export as undefined in production. This file must
// have no static path to an agent SDK, and nothing behind the driver
// registry may import it back either, even dynamically; AUTO_START_HOOKS
// below is how a driver's tool call reaches this sweep instead. Pinned by
// the DYNAMIC_ONLY and acyclicity cases in tests/importGraph.test.ts.
...
export function maybeAutoStartDependents(clearedTaskId: string): void {
  ...
  // Per-task LiteLLM virtual keys (docs/AGENTS.md, "Per-task virtual
  // keys"): every non-terminal to terminal transition converges here (the
  // PATCH route, both agent tools, lib/reclaim.ts and lib/tagRefresh.ts all
  // call this the moment they make one), so a live key is deleted here
  // instead of at each of those five call sites. Fire-and-forget: an HTTP
  ...
  // on top of it, so the cause travels instead of being flattened.
...
/**
 * What every turn launch hands the driver so a tool call that clears a
 * blocker can reach this sweep, the one thing a driver needs from this
 * module and must never import directly (lib/agents/types.ts's TurnHooks
 * explains why). Every startTurn / startResumeTurn caller passes this same
 * object: POST /api/tasks/[id]/messages, lib/dispatch.ts (runbooks and
 * schedules), lib/deferredStart.ts, and the launch below. A new launch path
 * that forgets it doesn't break its own turn, the tools still work, but it
 * leaves the dependents of anything the agent marks done sitting unstarted,
 * so pass it.
 */
...
export async function launchInitialTurn(taskId: string, note: string, admit: (fresh: Task) => boolean): Promise<boolean> {
  ...
  let launched = false;
  // Set as soon as there is a generation to report a failure against, which
  // is BEFORE the row is marked `running`, so a throw from the worktree
  // self-heal below still has somewhere to unwind to, not just a launch
  // failure after running=1. Null while there's nothing to undo.
  ...
  try {
    ...
    await withTaskLock(taskId, async () => {
      ...
      // expected fallback to repo_path. THROWING is different: a stale
      // index.lock from a crashed process, a disk-full git op, a detached
      // HEAD, and nobody is watching an auto-start launch, so swallowing it
      // here would run the turn unattended in the user's real checkout
      // instead of an isolated worktree. So it isn't caught here; it escapes
      // to this function's own catch below, which already turns a launch
      // failure into a visible transcript line via publishTurnError and
      // unwinds the row to retryable, the same path a broken runner import
      // takes.

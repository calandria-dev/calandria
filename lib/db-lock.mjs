 ...
 * SQLite database (calandria.lock.db, separate from the real one so a
 * concurrent `sqlite3 calandria.db` inspection still works and WAL is
 ...
 * both go through lib/storage.mjs. That sharing is load-bearing now that the
 * default location has a legacy fallback: a lock taken in a directory the app
 * then decides not to use would guard nothing.
 */
 ...
 * that adopted its database contend for the SAME file. Neither holds data we
 * ever read; the .json sidecar is diagnostics only, never load-bearing.
 */

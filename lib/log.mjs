/* One line-emitter for the whole app, so logs are readable by a machine
 ...
 *
 * text matches the historical `[component] message key=value` format so
 * existing `docker logs | grep` usage keeps working; set
 * CALANDRIA_LOG_FORMAT=json for a downstream log collector.
 *
 * Plain .mjs, zero imports, so both plain-Node entrypoints (server.js,
 * pty-server.js) and the TS modules under lib/ share one implementation.
 * Must stay SDK-free (tests/importGraph.test.ts) and is COPY'd into the
 * runtime image (Dockerfile), since Next's build output excludes it.
 *
 * CALANDRIA_LOG_FORMAT is read directly off process.env, not through
 * lib/env.mjs's readEnv, which only resolves pre-rename ORCH_* spellings
 * that this variable never had.
 */
 ...
 * `"text"`, the documented default; a non-empty typo logs a warning once
 * instead of serving a format nobody asked for with no explanation.
 *
 ...
 * otherwise turn "log this" into a second, uncaught failure inside the
 * handler that exists to report the first one.
 */
...
/**
 * One rendered line, without the trailing newline. Pure, and exported so
 * the shape can be pinned by tests without capturing console.
 *
...
/**
 * A logger tagged with one component name: the `[component]` prefix in text
 * mode, and a queryable field in JSON mode.
 *

// Which branch a task is based on, and how to change it. A project has one
// DEFAULT base branch (`projects.branch`), but a task may name its own
// (`tasks.base_branch`): the branch it was cut from, Sync catches it up to,
// and Merge lands it into, letting some tasks land on `feature/auth` while
// others ship to `main` from one project. See docs/FEATURES.md.
//
// 1. Every call site that used to write `project.branch` writes
//    `resolveBaseBranch(task, project)`; the git layer never learns about
//    any of this.
// 2. Inheritance stops AT THE WORKTREE CUT. `ensureWorktree` reports the
//    base it actually used and the launch paths pin it into
//    `tasks.base_branch`, so a later default change can't move a started
//    task's merge target out from under its pinned `base_sha`.
//
// Pinned SDK-free by tests/importGraph.test.ts: behind the task routes and
 ...
 *
 * The middle leg makes a tag the place a whole plan's base is configured
 * once instead of N times. A task can carry several tags, and two of them
 * may disagree; the tie-break is **the first tag by `task_tags.position`
 * that sets a non-empty base**, which is the order `getTaskTags` returns,
 * the order the badges render, and the order lib/tagContext.ts injects the
 * context blocks in, so "the tag this task is mostly about" is a fact the
 * row already carries. It is resolved instead of refused (adding a second
 * tag must never be able to break a task's next turn), and the tag strip
 * says when a member's base comes from a different tag, so nothing appears
 * out of nowhere.
 *
...
/**
 * The default this task inherits from its tags: the first non-empty one in
 * tag order, or "" when none of them sets one. Exported because the surfaces
 ...
 *
 * One function behind the route and the `set_base_branch` agent tool,
 * because the two would otherwise drift: this is asynchronous, touches git,
 * can create a local ref, can `reset --hard` a worktree and can fail
 * halfway, none of which belongs in `update_task`'s synchronous, atomic
 * field-writer.
 *
 ...
 *
 * `callerTaskId` is the session making the call: a task may retarget ITSELF
 * mid-turn, which the tool exists for, but not some other task whose turn
 * is live, mirroring `updateTaskForAgent`.
 */
export async function retargetTaskBase(
  ...
  // 1. Name. Before any git runs, so an unusable name is refused BY NAME
  //    instead of through whatever the first subprocess happens to say.
  ...
  // 3. Liveness. Re-read instead of trusting the caller's snapshot: a detached
  ...
  //    cut, clean tree. Re-cut instead of renumber: nothing can be lost, and
  ...
  if (ahead === 0 && !dirty) {
    ...
    // ordinary path instead of reporting a re-cut that didn't happen.
  }
  ...
  //    alone instead would keep a snapshot on neither branch's line and

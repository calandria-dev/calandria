...
// real checkout. On its own that leaves a bare "Could not prepare an isolated
// worktree: fatal: ..." with nothing to click, and a scheduled run that
// settles `failed` the same way every morning until someone goes digging.
//
...
export type WorktreePrepKind =
  ...
  | "detached_head"
  /** Not one that is recognised; the raw error is all that can honestly be said. */
  | "unknown";
...
// do ("fatal: Unable to create '.../index.lock': No space left on device"), so
// it must be recognised first or it would read as a stale lock and offer a
// Repair button that deletes a lock nothing left behind and then fails
// identically.
const DISK_FULL_RES = [
  /\bENOSPC\b/,
...
export function classifyWorktreePrep(msg: string | null | undefined): WorktreePrepDiagnosis {
  ...
  // and fail again on the next turn.
 ...
 *
 * Gated on WORKTREE_PREP_PREFIX instead of classifying any error text that
 ...
 * call inside the worktree), and that failure has nothing to do with
 * preparing the checkout, so offering to re-cut it would be the wrong advice
 * on the one line the user reads. Every prep failure carries the prefix,
 * because WorktreePrepError is the only thing that raises one.
 */
 ...
 * `recoverable` answer, though they still can, from the message alone, which
 * is what the runner does for failures that reach it as persisted text.
 */
...
/** True when `err` is a wrapped worktree-prep failure, instead of any other

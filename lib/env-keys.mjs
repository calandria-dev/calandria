// Boot-time guard against inherited billing credentials.
//
// The Agent SDK's `claude` children, the `codex` CLI and every pty shell
// inherit the server's environment. If ANTHROPIC_API_KEY leaked in from a shell
// profile, environment.d or a systemd unit, every turn would bill that key
...
// pty-server.js are separate processes, so each calls it).
//
// Post-strip invariant: an agent key in process.env is always intentional,
// either re-applied from the persisted 0600 file (lib/anthropic-key.ts /
// lib/openai-key.ts at db init) or kept via the CALANDRIA_ALLOW_API_KEY_ENV opt-in.
...
export function allowApiKeyEnv(env = process.env) {
  const v = String(readEnv("CALANDRIA_ALLOW_API_KEY_ENV", env) || "").toLowerCase();

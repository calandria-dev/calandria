...
// per repo so only one main-tree mutation runs at a time.
//
// The lock is keyed on the repo's COMMON GIT DIR, not on the path the caller
// handed us. Callers pass `project.repo_path` verbatim, and two spellings of the
// same directory — a symlink (`/Users/me/code` → `/Volumes/work/code`, macOS's
// `/tmp` → `/private/tmp`), a trailing slash, or simply two projects configured
// against the same repo with different base branches — would otherwise take two
// different locks and run concurrently, which is precisely what this exists to
// prevent. Git resolves realpaths; the lock has to agree with it. `rev-parse
// --path-format=absolute --git-common-dir` is the identity git itself uses: one
// answer per repository, shared by the main checkout and every linked worktree
// of it, already canonicalized.
//
...
// wedged subprocess from parking a task launch forever. A timeout falls back to
// the path form, same as a non-git directory.
...
async function gitCommonDir(repoPath: string): Promise<string> {
  ...
  try {
    ...
    // `--path-format` needs git ≥ 2.31; anything older errors out into the catch.
    // The absolute check is belt and braces against a relative answer we'd then
    // be keying on ambiguously.
 ...
 * same repo queue and run one at a time, in arrival order; different repos never
 * block each other. `fn`'s result (or rejection) is returned to its own caller —
 * a failing critical section never poisons the ones waiting behind it.
 *
 * Different paths naming the same repository — including a linked worktree of it
 * — resolve to the same key and therefore the same lock.
 *
 * Acquisition is no longer synchronous with the call (the key is resolved first),
 * so "arrival order" means the order callers finish resolving. Nothing depends on
 * stricter fairness than that, and the cache makes it the call order in practice.
 */

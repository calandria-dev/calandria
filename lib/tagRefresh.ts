...
//
// It is the tag analogue of lib/contextRefresh.ts and copies its shape
// (detached job, state on the row, polled by GET), but the context draft
// produces a DRAFT the user reviews, while this one APPLIES its outcome. Every
// task change goes through lib/agentTools.ts and is recorded in
// task_agent_edits, so the user gets a "Changed by agent" chip with a
// per-field before/after and a one-click Revert; review happens after the
// write, on that existing surface.
//
...
// happen, keeping the judgement and the write apart so the audit trail is real.
...
// Tags whose refresh is executing in THIS process. Same contract as
...
// one reads it once per member task it has doubts about, and a tag with a
// dozen members can take a while.
...
function stateOf(t: Tag): TagRefreshState {
  ...
  // can retry instead of watching a bar that will never move again.
...
/**
 * Acknowledge a finished run: clear the summary/error back to idle so it
 * doesn't resurface every time the chip is lit. No-op while a job is running.
 ...
 * to judge "already overtaken" honestly: a started task has a checkout and a
 * diff, and naming that stops it proposing to retire real work. This module
 * needs the same distinction to decide what it may retire.
 */
...
/**
 * The digest handed to the agent: the tag, its saved description, every
 * member's brief, and recent git activity. Descriptions are included, unlike
 * list_tags's shallow view, because judging whether a brief has gone stale
 * cannot be done from a title.
 */
...
/**
 * Apply a parsed plan. Exported for the tests, and takes everything it needs
 * as arguments instead of re-reading, so the policy below is assertable
 * without an agent in the loop.
 *
 * What it does:
 *   - rewrites the tag's description;
 *   - rewords a member's title/description, through updateTaskForAgent, so it
 *     lands as a revertable agent edit;
 *   - retires a member that has no work in it: an unreviewed suggestion (via
 *     the ordinary withdraw path, which leaves it struck through in the tray)
 *     or a task the user accepted but never started. Cancelling either
 *     destroys nothing, and Revert puts it back exactly.
 *
 * It will NOT retire a STARTED task: that task has a checkout and probably a
 * diff, and reading the main branch can't reveal what is in it. Those are
...
export function applyTagPlan(tag: Tag, members: Task[], plan: TagPlan, actor: AgentEditActor): TagRefreshOutcome {
  ...
  for (const entry of plan.tasks) {
    ...
    // task, and either way isn't what the user pressed the button about.
    if (!member) {
      out.ignored++;
      continue;
    }
  ...
  // A retirement is a non-terminal to terminal transition, so a dependent
  // parked on "start when unblocked" may now be free. Runs the same sweep the
  // tool paths run.
  if (cleared.length) {
...
// The one line under the bar. It says what happened, including "nothing": a
// refresh that found a healthy plan has done its job and must not read like a
...
// reconnecting minutes later still gets the report.
async function runRefresh(project: Project, tag: Tag): Promise<void> {
  try {
    ...
    // Re-read the members: the exploration takes minutes, and a sibling
    // session can start, finish or cancel one of them while it runs. The plan
    // is a judgement about ids, not about the rows the job opened with.

 ...
 * hosted-MCP selection (docs/AGENTS.md). Codex gates MCP calls with its own
 * per-server approval mode, which the turn's approval_policy doesn't reach,
 * so every entry here also carries `default_tools_approval_mode: "approve"`.
 * This function is called by the driver under every permission mode but
 * "plan" (see lib/agents/codex/driver.ts), the read-only mode these tools
 * would contradict.
 */

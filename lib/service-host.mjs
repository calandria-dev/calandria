 ...
 * host of PUBLIC_BASE_URL (e.g. calc--ishan.calandria.example.com for the instance at
 * https://ishan.calandria.example.com). The scheme is FLAT on purpose: Cloudflare
 * Universal SSL only covers *.calandria.example.com one level deep, so a nested
 * calc.ishan.calandria.example.com would need paid Advanced Certificate Manager.
 ...
 * managed-services feature itself: OFF unless CALANDRIA_SERVICE_HOSTS is set
 * truthy. The services feature ships enabled by default, and enabling it must
 * never expose anything on a public hostname — an instance additionally needs
 * this env AND a PUBLIC_BASE_URL (plus wildcard DNS/TLS for *--<host>) before
 * the router (lib/service-router.mjs) claims any hostname.
...
// as the ?t= handoff from /api/services/grant and as the calandria_svc cookie on the

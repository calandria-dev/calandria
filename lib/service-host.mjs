 ...
 * host of PUBLIC_BASE_URL (e.g. calc--ishan.calandria.example.com for the
 * instance at https://ishan.calandria.example.com). The scheme is flat:
 * Cloudflare Universal SSL only covers *.calandria.example.com one level
 * deep, so a nested calc.ishan.calandria.example.com would need paid
 * Advanced Certificate Manager. "--" is the separator, so service slugs may
 * never contain it.
 *
 * Plain ESM JS (like lib/auth/origin.mjs), imported from three worlds:
 * server.js (raw Node, pre-Next), Next route handlers, and vitest.
 */
 ...
 * managed-services feature: OFF unless CALANDRIA_SERVICE_HOSTS is set truthy.
 * The services feature ships enabled by default, but enabling it alone
 * exposes nothing on a public hostname; an instance also needs this env AND
 * a PUBLIC_BASE_URL (plus wildcard DNS/TLS for *--<host>) before the router
 * (lib/service-router.mjs) claims any hostname.
 * @param {Record<string, string | undefined>} [env]
 */
export function serviceHostsEnabled(env = process.env) {
  const v = readEnv("CALANDRIA_SERVICE_HOSTS", env);
...
export function parseServiceHost(hostHeader, appHost) {
  ...
  // A name containing "--" (e.g. a--b--ishan.…) parses ambiguously, so it is
  // rejected here and never issued at registration (slugifyServiceName
  // collapses runs).
...
// ---------- gate tokens ----------
// The short-lived credential a browser holds for a private/shared service,
// both as the ?t= handoff from /api/services/grant and as the calandria_svc
// cookie on the service hostname. HMAC over (slug, expiry) with an
// instance-local secret, minted and verified on the same box, so no PKI is
// needed.

export const GATE_COOKIE = "calandria_svc";

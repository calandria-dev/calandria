...
//
// The bare name "codex" is fine on POSIX (execvp searches PATH) and broken on
// native Windows, where the npm global install writes `codex.cmd` and nothing
// named `codex`: CreateProcess never finds it. mcp.ts is the expensive half,
 ...
 * which on POSIX is the same behavior either way, since a PATH hit there
 * resolves to the same file execvp would have found.
 */

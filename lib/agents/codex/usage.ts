...
export interface CodexTokenUsage {
  ...
  // Optional so an older `codex` binary that omits this field prices as
  // plain input.
 ...
 * buckets disjoint (matching Claude's), so they're netted out here instead of
 * double-counted in the task total and the context gauge. Reasoning output folds
 * into output_tokens, since the API bills reasoning as output. Callers pass
 * per-turn counters: events.ts subtracts the thread's cumulative baseline
 * first, and the auth verify parser reads a fresh single-turn thread.
 */

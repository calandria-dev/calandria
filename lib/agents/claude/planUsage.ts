...
//      threshold is crossed. Below that it reports only "allowed", which
//      window binds, and when it resets. The CLI holds a per-window
//      utilization map internally, feeding its statusline, but never puts it
//      on the SDK wire.
//
...
//
// The passive signal overlays the cached fetch: a warning or rejected event
// from a live turn updates its window's percentage and the status trio
// immediately, so "limit reached" never waits out the fetch floor.
//
// SDK-free (fs + fetch only, pinned by tests/importGraph.test.ts) and
// instance-wide on globalThis: one Claude login per instance means one
...
// Separate from the success floor so a flapping endpoint isn't hammered at the
// poll rate, but recovery doesn't wait out a full success interval either.
...
// than a dropped limit when the provider adds a window.
...
// Which of the two windows every metered plan has each key belongs to: what
// the pill and the queued-start button pick by, so neither has to know one
// provider's spelling of "the 5-hour one". The per-model weeks are absent
// here: they are extra rows, not the week the plan is paced against.
...
// seconds. Tolerate milliseconds defensively (values consistent with an epoch
// in ms terms), the same heuristic as the driver's withResetTime().
 ...
 * turn's stream carries. Cheap and synchronous, since it must never slow the
 * pump.
 */
export function recordClaudeRateLimit(info: unknown): void {
  ...
  // already-percent value defensively, since a fraction can't exceed 1.
...
// The response shape the CLI's /usage panel renders: top-level
// `{utilization, resets_at}` objects under the WINDOW_LABELS keys, utilization
// already percent 0-100, plus a `limits` array whose `weekly_scoped` entries
// carry per-model weeks as
...
export async function getClaudePlanUsage(): Promise<PlanUsageSnapshot | null> {
  ...
  // only one it names), and a percentage is only ever moved by newer data. A
  // passive figure whose own reset has passed is stale by definition, since the
  // window rolled over and its utilization snapped back, so it must not
  // overlay: a healed 100% would otherwise alarm until the fetch floor allowed
  // a correction.
  ...
  if (p && passiveCurrent && p.at >= (st.fetched?.at ?? 0) && p.window && p.utilization != null) {
    ...
    if (hit) {
      ...
      // No fetch has succeeded (or the endpoint doesn't list this window). The
      // passive figure alone still beats showing nothing.

...
// (lib/agents/types.ts). runTurn() drives one user turn (resume or fresh
// session; project context appended to the Claude Code system prompt),
// mounts the Calandria MCP tools (suggest_task / list_projects / list_tasks /
// get_task / update_task / withdraw_suggestion / expose_service), and
...
// the SDK default is "when omitted, all sources are loaded, matches CLI
// defaults", so an SDK bump could otherwise change what a task trusts.
// Written out here so there is a line to grep for and a test to fail
// (tests/claudeSettingSources.test.ts).
//
// Each source is a JSON file the SDK merges in, and none of the keys it can
// carry are inert: `hooks` runs shell commands on tool/session events,
// entirely outside canUseTool, so the permission gate never sees a hook
// fire. `permissions.allow` is auto-approved with no canUseTool call at all,
// and `env` reaches every subprocess a tool spawns. Loading a source hands it
...
// task's worktree is where an agent's own writes land, so this file is as
// writable as anything else the agent touches. It stays in because it's
// tracked: it shows up in the same diff (TaskChanges) the user reviews before
// anything merges, so a hook smuggled in here is one the review step catches.
//
// 'local' (<worktree>/.claude/settings.local.json) is dropped. It resolves
// against the same worktree, so it's as agent-writable as 'project', but by
// convention it's gitignored and never appears in that diff. An agent running
// under an auto-accept edit policy (Write pre-approved, no canUseTool
// round-trip) can create this file with a PreToolUse hook, and the very next
// turn that hook runs as a shell command with no permission check in
// between: the classifier and the permission card are both bypassed, because
// settingSources loaded the file before either had a chance to look at it.
// 'project' is worktree-writable too, but a human reviewing the diff has a
// shot at catching it; 'local' guarantees they never see it.
//
// These files are re-read from disk on every turn, so a worktree's
// settings.json can drift between the turn a human reviewed and the turn
// that runs next; that gap is closed by WATCHED_SETTINGS_FILES below, not by
// narrowing this list.
//
// Exported because it is used outside this file too: the schedule preflight
// (lib/schedule/commands.ts) opens a throwaway session purely to read the
// slash-command registry a scheduled turn would get, and validates against
// it. A second hardcoded copy there could drift from this one, and a drifted
// registry does not degrade gracefully: an unknown command settles the run
// `failed` and mints nothing. Pinned by tests/claudeSettingSources.test.ts,
// which drives the real probe through the mocked SDK and reads the sources
// back.
...
//
// A total Record over the SDK's SettingSource union, so the mapping can't
// fall behind the union: if a future SDK adds a tier, this stops compiling
// and somebody has to say where it lives and whether a task can write it,
// instead of the new source joining SETTING_SOURCES unwatched. 'user' is
// ~/.claude/settings.json, the operator's own machine, outside every
// worktree and no more trusted than the person running Calandria, so it
// maps to null and is not watched: it changes when the operator changes it,
// and holding a turn on that would be a card raised against yourself.
...
//
// Derived from SETTING_SOURCES instead of written out: "which sources a
// turn loads" and "which files drift detection covers" are one fact with one
// source of truth, so re-adding 'local' to the list above extends the gate to
// it in the same edit instead of reopening the hole under a second, silent
// name. Pinned by tests/claudeSettingSources.test.ts, which asserts the
// derivation, not the current value; the runner half is
// tests/settingsDrift.test.ts.
...
// Shared with command discovery: `.claude/commands` in the checked-out repo
// is part of the answer, so the menu is accurate only when it's rooted where
// the turn will be.
function sessionCwd(task: Task, project: Project): string {
  return task.worktree_path || project.repo_path || process.cwd();
}

// One-shot runs (handoff note, recap, etc.) get a different policy than a
// user session. They have no Calandria bridge, no transcript and no way to
// answer a prompt, so inheriting the full session config only spawns the
// user's entire MCP fleet to offer tools the job can never use.
//
// The one-shots isolate CAPABILITY and inherit CONFIG, the same split the
// Codex driver's oneShot() makes (read-only sandbox, no network, the user's
// MCP servers unmounted, but ~/.codex/config.toml still read for auth and
// model). Four levers do it:
//
//   tools:           the real restriction. `allowedTools` is not one: the SDK
//                    defines it as "auto-allowed without prompting", and under
//                    bypassPermissions everything is auto-allowed anyway.
//   strictMcpConfig: drops MCP from settings, .mcp.json and plugins. `tools`
//                    alone doesn't, since it governs built-ins only.
//   settings:        inline overrides, merged over whatever the sources below
//                    loaded. `disableAllHooks` matters because the user's
//                    hooks live in ~/.claude/settings.json and a SessionStart
//                    hook injects context whether or not any tool exists to
//                    hook. (`managedSettings` does not work here: the SDK
//                    filters that tier restrictive-only and the key doesn't
//                    survive; it also impersonates the IT policy tier, which a
//                    real one would displace.) `autoMemoryEnabled: false`
//                    keeps the user's private per-project memory out of an
//                    internal transformation.
//   settingSources:  kept at ["user"], not []. ~/.claude/settings.json is also
//                    where a user's `env` block, `apiKeyHelper` and model
//                    aliases live, so it matters for auth and provider
//                    routing, not just for MCP and plugins. On a
//                    Vertex-configured machine with those vars absent from the
//                    server's own environment, `[]` fails the run with "Not
//                    logged in" while `["user"]` succeeds with 0 tools and 0
//                    MCP servers. Isolating here would break recap and /clear
//                    for every Bedrock / Vertex / proxy / apiKeyHelper user
...
const ONE_SHOT_BASE = {
  ...
  // The Skill tool isn't in any one-shot's `tools`; this guards against the
  // discovery pass finding it anyway.
  skills: [] as string[],
  ...
  // A one-shot has no UI to answer a permission card, so if a future edit
  // adds a tool it must not start prompting into the void.
...
/**
 * Every tool on the server above goes through lib/agentToolGuard.mjs, applied
 * to the array instead of written into each handler: a tool added later
 * cannot forget it, and the stdio bridge wraps its own registrations with the
 * same module, so the two ends of the seam can't drift apart.
 *
 * The guard only replaces an answer that isn't one: a throw, a call that ran
 * past its bound, or an empty result. A handler that returns normally is
 * passed through untouched, `isError` included.
 *
...
function calandriaServer(
  ...
  // planning a roadmap. Keyed by (target project, title): a suggestion can be
  // filed into any project and dependencies never cross one, so the same
  // title in two projects is two unrelated tasks, not an ambiguous ref.
    ...
    tools: guardTools(task.id, [
      ...
      tool(
        ...
        async (args: { title: string; description: string; priority: "hi" | "med" | "lo"; project?: string; blocked_by?: string[]; tags?: string[]; provider?: "local" | "cloud"; model?: string }) => {
          ...
          // failure. Strict: an unrecognized name is refused, never treated
          // as "here".
          ...
          const { task: created, text } = createSuggestedTask(target.project, {
            ...
            // createSuggestedTask, so a cross-project suggestion tags where it
      ...
      tool(
        ...
        UPDATE_TASK.description,
        {
          ...
          // Spelled out instead of read from UPDATE_TASK.priorities/.statuses:
      ...
      tool(
        ...
        async (args: { tasks: string[]; project: string }) => {
          ...
          // No discard acknowledgement is accepted here: a started task's
          // checkout can only be destroyed with the user's per-task answer,
          // which they give from the board.
        ...
        ? [
            tool(
              ...
              async (args: { title?: string; body?: string }) => {
                ...
                // caller, and lib/prTools.ts re-reads it, since this turn's
                // snapshot predates its own worktree cut.
...
// Maps the UI reasoning preset to the SDK's thinking controls. `maxThinkingTokens`
...
// on/off, so `effort` is scaled too, to keep higher presets visibly thinking more
// there. null inherits Claude Code's default (no override).
...
// than derived from CLAUDE_CAPABILITIES because that module is SDK-free and
// types its values as plain strings, the same trade the MCP tool enums above
// make. Pinned the same way: tests/claudePermissionMode.test.ts asserts this
// list and the picker's list are exactly each other.
...
//
// The default is "auto", not bypassPermissions, because canUseTool is a real gate: the
// classifier approves what it judges safe and escalates only the rest to a
// card, so a task is screened without a click per Read. The cost is on
...
// Stop button and the SDK's own per-request cancellation. It must answer to
// both, and leaking the listeners would pile up one per prompt on a long turn.
...
async function* runTurn(
  ...
  // the pending-ask registry keys by id, so a shared fallback would collide.
  let askSeq = 0;
  ...
  // call not yet in here is a foreground completion: the CLI announces every
  // Bash/Agent task it registers (task_started + task_notification, summary =
  // the call's own description, then the tool_result in the same instant),
  // and the card is about to carry the result, so the line would only repeat
  // the description. One that is in here settled a backgrounded call: its
  // card holds a "running in the background" placeholder, so the
  // notification is the only news.
  ...
  // does not exist and Claude can never ask.
  //
  ...
  // already covered by a remembered project rule pass without a prompt;
  ...
  // --- Background linger ---
  //
  // In single-prompt mode the CLI exits shortly after the result message and
  // kills every run_in_background child with it, so "you will be notified
  // when it completes" never happens. Streaming-input mode fixes both halves:
  // with the prompt iterable held open the CLI stays alive after the result,
  // background tasks run to completion, and their task_notification
  // re-invokes the model into a fresh turn with no user message, which
  // streams through this same pump as a continuation of the generation.
  //
  // The turn-end signal is the Stop hook's `background_tasks` payload, the
  // SDK-documented field for distinguishing "session is done" from "session
  // is paused waiting for background work to wake it". It fires before every
  // result message, so by the time the pump sees a result it already knows
  // whether work is pending. (The CLI also emits a `background_tasks_changed`
  // system message with the same array, but that subtype exists nowhere in
  // sdk.d.ts, so it is not relied on here.) `session_crons` (ScheduleWakeup /
  // CronCreate / /loop) rides the same Stop payload and is the second linger
  // trigger: a cron fires only while the CLI is alive, and closing the input
  // exits it with the wake simply gone. Held open, the wake arrives as a bare
  // second `init` (same session id, no user message, no task_notification),
  // so it gets its own wake accounting below. Which crons are honored is
  // planSessionCrons()'s decision: everything under the unbounded default (a
  // recurring /loop cron holds the session open until the user stops it,
  // visible on the row), only what fits the window on a bounded instance.
  // Whatever is not honored is named in a transcript notice when the input
  // closes (endTurn), so neither the user nor the model's next turn waits on
  // a wake that died with the process.
  //
  // Closing the held-open iterable is the whole teardown: the CLI gives
  // still-running tasks a grace period, kills them (task_updated
  // status:"killed"), and exits, so Stop, the SIGTERM drain, and the
  // (optional) linger deadline all converge on closeInput(). By default there
  // is no deadline: the lingering state is visible in the UI with its age, so
  // a session held too long is the user's to kick (Stop), not the harness's
  // to kill. When an operator sets BACKGROUND_LINGER_MS > 0, that deadline
  // bounds the whole linger phase from its first entry (one timer, never
  // reset by wake turns): a deadline reset per wake would let a task chain
  // sleeps forever, and one cleared on wake would hang the query if a
  // notification ever arrives without a wake turn behind it (skip_transcript
  // housekeeping tasks).
  ...
  // and one notice per wakeup is honest; one per Stop would be noise.
  ...
  // running session total, so a wake turn reports the first result's cost
  // plus its own marginal spend. Report the delta or every wake turn
  // re-bills the whole session. A total below the baseline is taken at face
  // value, not clamped, on the chance the report is per-turn after all.
  let costBaseline = 0;
  ...
  // becomes several messages carrying the same usage. Where the gauge can
  // ignore an unchanged number, spend summed per message would bill that
  ...
  // unbounded by default. A message pushed after the first result is accepted
  // and starts a fresh turn on the same session, announced by a bare second
  // `init`, the same shape a cron wake takes, with no user echo on the wire
  // either, which is why the wake branch below must not read an injected
  // turn's init as a wakeup. Closing the channel is what ends the query,
  // exactly as the latch did.
  ...
  // the last Stop hook reported is about to die with the process: whichever
  // path closes (nothing honored at result time, the deadline, Stop, a
  // transport error), a cron still pending at that moment will never fire,
  // and the notice says so once. A caller that has already named the crons in
  // its own notice passes `null`.
  ...
  // flight, and the message opens the next turn. Mid-thought it would arrive
  // in the middle of the model's reasoning, which is what pending_messages is
  // for. Refusing (false) is the caller's cue to queue.
  //
  ...
  // Re-lingering after this keeps the injected turn from costing the user the
  // work they were waiting on: the injected turn's own Stop hook reports the
  // still-running background task (and a pending wakeup) exactly as the
  // previous one did, so the result handler below re-enters the linger and
  // the work runs to completion.
  const sendMidTurn = (text: string): boolean => {
    ...
    // only) starts over instead of counting a wait the user just ended: clear
  ...
  const response = query({
    ...
    options: {
      ...
      // that died), which a turn's journal otherwise has no record of.
      stderr: (data: string) => {
      ...
      mcpServers: {
        // Hosted LiteLLM gateway MCP servers (docs/AGENTS.md, "Hosted MCP
        // servers"), the project's selection (or this task's override).
        ...
        calandria:
          CLAUDE_TOOL_TRANSPORT === "stdio"
            ...
            : calandriaServer(
                project,
                task,
                // Straight onto the queue, like expose_service's notice below:
                // the suggestion is already committed, and holding it until
                // the turn ends would keep the receiving tray stale for as
                // long as the turn runs, hours if it parks on a question.
        ...
        PreToolUse: [
          {
            ...
            hooks: [
              async (input, toolUseId) => {
                ...
                // permission-card machinery, not the ask machinery: an ask
                // card implies an answer is coming, while a decided
                // permission card is exactly what this is, a request that was
                // refused, on the record, with the question preserved so the
                // user can see what the run wanted. It also lands the refusal
                // where the runner already watches, so the run settles
                // `failed` with an actionable reason instead of a green "ran".
                ...
                try {
                  ...
                  // would render live option buttons forever, indistinguishable
                  // from a question somebody is actually waiting on.
  ...
  // Pumps SDK messages into the queue. Runs concurrently with the hook, which
  // pushes ask events while this is parked awaiting the tool result.
  const pump = (async () => {
    try {
      for await (const message of response) {
        if (message.type === "system" && message.subtype === "init") {
          // A wakeup firing mid-linger arrives as exactly this: a second init
          // on the same session, with no user message and no
          // task_notification, so this is the cron wake signal. Only claimed
          // when crons were being waited on: a background task's wake is its
          // notification, which precedes its init (pinned by the linger
          // test), so that path has already left the lingering state by the
          // time its init streams. A message the user sent mid-linger
          // produces an identical bare init and is excluded the same way:
          // sendMidTurn() drops `lingering` in the same tick it accepts the
          // message, so the wake it announces is the user's, not a wakeup's,
          // and the transcript already shows their message.
          ...
          // "auto" that's the classifier vetoing a call it judged unsafe; it
          // can also be a deny rule in the loaded settings. The only other
          // trace is an is_error tool_result the transcript renders as a
          // plain tool failure.
          //
          // Carries the tool_use id, so the runner can settle this onto the
          // transcript card the call already created: it reads as "this call,
          // refused, here's what it was" instead of a notice floating beside
          // it, and three denials in a turn are three decided cards, not
          // three identical loose lines.
          queue.push({
          ...
          // their own window, so they're skipped; a synthesized error message
          ...
          if (message.parent_tool_use_id == null) {
            ...
            // half an hour of tool calls without producing one, so a Stop deep
            // into that segment would otherwise write zero tokens for work the
            // API had already billed. These are provisional: the result
            // message's usage is exactly the sum over the main-session
            // assistant messages of its segment (see claudeSubagentTokens), so
            // the runner drops what it accumulated here the moment a full
            // report arrives, instead of billing both. Sidechains are excluded
            // for the reason above and because summing their messages
            // undercounts by half; a Stopped turn's subagent spend is
            // therefore unrecorded, the same floor the stored token totals
            // have always had.
            const seen = {
          ...
          for (const block of message.message.content) {
            if (block.type === "text" && block.text.trim()) {
              ...
              // The tool's own name travels with the row, so the runner can
              // match on it to settle a suggestion card onto a suggest_task
              // call instead of matching the title, which is human prose.
          ...
          if (Array.isArray(content)) {
            for (const block of content) {
              if (block && typeof block === "object" && (block as { type?: string }).type === "tool_result") {
                ...
                // or a turn whose Calandria calls all failed would still
                // report `turn ok` with nothing in the journal to find it by.
                ...
                if (cut && cutOff) {
                  ...
                  // here, but the user can start a fresh session instead
                  // (lib/agentToolGuard.mjs, toolCutoffNotice).
                ...
                // non-zero exit sits at the end, and a head-only clip would
                // drop it, leaving a long failed command reading as good
                // output under a red ✗ with nothing to explain it.
          ...
          // streams through this same pump, so surface the transition so the
          ...
          // token counts, but a cumulative session dollar total, so delta it
          ...
          // Those token counts are the main session's alone, while the dollar
          // figure beside them covers the subagents too. Measure the gap so
          // the chip can say so instead of reporting a fan-out as if this
          // session had burned it (see claudeSubagentTokens).
          ...
          // session is done, close the input and let the CLI exit (naming any
          // wakeup that dies with it); something to honor means hold the query
          // open so the work survives, tell the runner what the session is
          // waiting on, and start the (optional) bounded wait for the
          // task_notification / wake-init.
          if (!closing) {
            const now = Date.now();
            ...
            if (bg.length > 0 || plan.linger.length > 0) {
              ...
              // is fixed from first entry), so say so now, beside the wait.
      ...
      // since a parked generator would otherwise pin this turn's closure
      // forever, and take the input channel off the registry so a message
      // arriving now is queued instead of pushed into a session that is gone.
 ...
 */
// The tier setting the caller resolved, in the shape query() wants. Omitted
// when unset, so a one-shot inherits Claude Code's own configured default.
...
// The draft agent's tools. Unlike the two summarizers this one needs to look
// at the repo, but only to look. Bash is absent: under bypassPermissions with
// no canUseTool it would be unreviewed arbitrary execution in the user's own
// checkout, for a job whose output is a paragraph of prose, and the
// recent-activity half it would be wanted for (git log) is handed in via
// `digest` already. Read/Grep/Glob cover reading files, searching content and
// walking the tree. `allowedTools` is not the right lever here: it restricts
// nothing, since the SDK auto-allows what it lists instead of limiting the
// toolset to it.
...
// can read. 'local' stays off: it adds only the gitignored CLAUDE.local.md
// and .claude/settings.local.json, one developer's private overrides leaking
// into a document written for everyone.
 ...
 * because this run's whole output is a JSON plan the server applies. The
 * agent deciding a task is stale and the app writing that decision down are
 * kept apart, so every change lands as a revertable agent edit instead of an
 * unattended write.
 *
 ...
 * statuses, recent commits). Describes what happened only, with no next-step
 * suggestions.
 */
...
export const claudeDriver: AgentDriver = {
  ...
  // settings, both read from disk (./provider.ts). GET /api/agents therefore
  // serves what a turn on this machine would really resolve, and picks up a
  // settings edit without a restart.
  get capabilities() {
  ...
  //
  // `null` there means "couldn't find out", a distinction the schedule
  // validator needs and the menu does not: an unreachable CLI costs the
  // composer its long tail and nothing else, so both failures land on the
  // same empty list.

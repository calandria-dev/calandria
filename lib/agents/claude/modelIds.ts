...
// better-sqlite3 path — so it cannot itself go and ask. ./modelProbe.ts does the
// asking, detached, and leaves the answer here.
//
...
// the prober directly would close that loop, and this repo has been bitten
// before by what Turbopack does with a cycle through an async external (see the
// TurnHooks note in CLAUDE.md). Data one way, no edge back.
...
// On globalThis for the reason lib/events.ts and lib/modelEndpoint.ts's probe
// cache are: HMR reloads the module, and an answer that costs a ~19s sweep of
// CLI spawns must not reset on every edit.
...
/** Forget the resolution — for a test, or a deliberate re-probe. */

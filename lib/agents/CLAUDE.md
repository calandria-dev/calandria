...
**Plan usage is an active read**, unlike Claude's free ride on `rate_limit_event` messages: the
Codex turn stream carries no token/rate-limit field and nothing is cached under
`$CODEX_HOME/sessions` either. `codex/planUsage.ts` reads `account/rateLimits/read` from a
throwaway `codex app-server` (`codex/appServer.ts`), behind `PLAN_USAGE_MIN_FETCH_MS`. Field names
come from `codex app-server generate-json-schema` and are camelCase (`usedPercent`,
`windowDurationMins`, `resetsAt` in seconds), windows named by rank (`primary`/`secondary`), not
duration.

Enterprise-managed approval requirements can disallow `approval_policy=never`, which the exec
transport can't survive; the driver detects the CLI's downgrade warning and self-heals to
`on-request`, recording `codex_approval_downgraded`.
...
A **Codex** task gets the Calandria bridge and nothing else by default. The SDK flattens
Calandria's config into leaf-level `--config mcp_servers.calandria.…` overrides, which the CLI
merges into `~/.codex/config.toml` alongside the user's own servers regardless. But `codex exec`
has no approver, so their tools would be offered to the model and every call would return
`"user cancelled MCP tool call"` (verified against codex-cli). `codex/mcp.ts` enumerates them via
`codex mcp list --json` and unmounts each (`enabled = false`), so no dangling uncallable tools
burn context. `default_tools_approval_mode: "approve"` stays scoped to
Calandria's own first-party bridge; it never becomes a global Codex setting. `CODEX_INHERIT_MCP=1`
opts back in.
...
Mirrors Codex's own `oneShot()`: read-only sandbox, no network, MCP unmounted, but
`~/.codex/config.toml` still read.

 ...
 *
 * - Access authenticates users at the edge, but anything that reaches the
 *   origin directly (loopback port, tunnel misconfig, another process on
 *   the box) would bypass it, and this app hands out a shell. So every
 *   request must also prove itself at the origin by presenting the Access
 *   JWT (the `Cf-Access-Jwt-Assertion` header Access injects, with the
 *   `CF_Authorization` cookie as fallback), verified against the team's
 *   public keys and this app's aud tag.
 * - One module, three consumers, kept in sync by keeping the logic here:
 *   middleware.ts (every HTTP route), server.js (WebSocket upgrades, the
 *   /pty terminal proxy), and app/api/auth/whoami/route.ts (reports whether
 *   a session exists, so the settings nav can offer Log out).
 * - Plain .mjs: middleware.ts loads it through Next's bundler (edge
 *   runtime, Web APIs only), server.js is unbundled CommonJS and loads it
 *   with a dynamic import(), and jose v6 is ESM-only, so this is the one
 *   shape that serves both.
 * - Config (see .env.example): CF_ACCESS_TEAM_DOMAIN (e.g.
 *   your-team.cloudflareaccess.com), CF_ACCESS_AUD (the Access
 *   application's aud tag, comma-separable), SERVICE_TOKEN (shared secret
 *   for the health-check bypass).
 * - Enforcement is on iff TEAM_DOMAIN and AUD are both set; local dev runs
 *   open by default, and the per-user containers set both (docs/DEPLOY.md).
 */
...
/** The non-Access paths: health/version/usage probes (Docker HEALTHCHECK and a
 * fleet dashboard hit /api/instance/usage and /api/version from inside or
 * alongside the container, where no Access JWT exists). Two accepted secrets:
 *
 *   SERVICE_TOKEN          the per-instance token (unique per box).
 *   CALANDRIA_FLEET_TOKEN  an optional fleet-WIDE read token shared by every box, so
 *                     a metrics dashboard can poll the whole fleet with one
 *                     secret instead of learning each box's private
 *                     SERVICE_TOKEN. Read-only paths only.
 *
...
export function serviceTokenOk(presented) {
  return (
    ...
    constantTimeEq(presented, readEnv("CALANDRIA_FLEET_TOKEN") || "")
  );
}

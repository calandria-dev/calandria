 ...
 * with zero code edits. Every value has a documented default; see
 * docs/SELF_HOSTING.md "Configuration" and .env.example.
 *
...
/*
 * The two on-disk locations resolve through lib/storage.mjs instead of a bare
 ...
 *
 * Declared here and not only in the script, since every per-instance knob
 * gets one documented default and one place to look, even though the app
 * itself never reads it: the backup runs as a plain-Node script that can't
 * import TS and reads the same env name directly, exactly as server.js does.
 * Point it at a different volume to keep backups off the disk they are
 * backing up.
 */
 ...
 * chooses to open it (lib/uploads.ts). 25 MB covers a log bundle, a design
 * PDF or a small archive, and stops the multipart body being buffered whole
 * in the server's heap, which is why the route also rejects on Content-Length
 * before parsing. Raise it if your instance has the memory.
 *
 ...
 * not usable, so a real `claude.exe` is required on Windows: this value goes
 * to the Agent SDK's `pathToClaudeCodeExecutable` and to node-pty for the
 * login, and neither offers a cmd.exe wrapper to route a batch shim through
 * (unlike the codex helpers, which shell out through child_process,
 * lib/binPath.ts's spawnSpec). `.exe` is ordered ahead of `.cmd` in
 ...
 * because the server process never reads a shell profile, so a gh that works
 * in the user's terminal (Homebrew, snap) can be invisible to the app's PATH.
...
// that have no meaningful "unset" default to prefer. Unlike ms(), a bad value
// here is loud: it's a typo in an env file, and left unguarded it would
// surface as a deep SQLite bind-type error on project creation instead of a
// named boot warning.
 ...
 * one to answer, so the prompt fails closed quickly instead of wedging the
 ...
 * its own. The CLI's per-call MCP timeout defaults to about 27.7 hours and
 * cannot be set per in-process server, so without this a wedged tool call
 * stalls the turn for the rest of the day. Read here for the Claude driver's
 * in-process server and directly from the environment by
 * scripts/calandria-mcp.mjs, which is plain Node and can't import this file;
 * keep the name in sync. 0 disables the bound.
 */
 ...
 * default) writes none. It is the one diagnostic for a Calandria tool call the
 * CLI answers itself (lib/agentToolGuard.mjs): that failure happens above the
 * seam every server-side record sits on, so only the CLI's own log says what
 * it did. Verbose and never pruned; turn it on to catch the next occurrence,
 * off once caught.
 */
 ...
 * completes", so ending the query at result time kills the work with no
 * warning. With linger on (the default), the driver holds the session open
 ...
 * starts lingering (wake turns don't reset it). Default 0 means no deadline:
 * the session waits until the work settles, the user presses Stop, or the
 * process shuts down. The lingering state is visible ("working in
 * background", with how long it's been), so a session held too long is the
 * user's call to kick, while an automatic cut would kill real work to enforce
 * a bound nobody asked for. Set a positive value to auto-stop lingering work
 * at that age instead: the work is killed and a transcript notice names what
 * was cut so the next turn doesn't assume it finished. The cost of an
 * unbounded linger is one idle CLI process and the task's turn slot; a
 * backgrounded process that never exits (a dev server) holds both until
 * stopped.
 */
 ...
 *
 * On by default. It is a directive, not a suggestion, because it
 * countermands one: the CLI's own auto-mode guidance asks for work to go
 * through Bash and says not to call the Agent tool unless the user requested
 * it. The same rule stated in a CLAUDE.md file fires later and less often than
 * one appended to the session prompt (https://github.com/calandria-dev/calandria-notes/blob/main/measurements/DELEGATION.md
 * has the comparison). Set to off/0/false to leave every session on the CLI's
 * defaults: the escape hatch for an instance whose plan makes subagent turns
 * expensive, or whose work is small enough that the dispatch overhead is the
 * whole cost.
 */
 ...
 * exists because a wedged wait and a working turn are otherwise drawn
 * identically, and with no linger deadline (BACKGROUND_LINGER_MS = 0, the
 * default) a wait on something that already finished holds the session open
 * forever with nothing to show for it.
 *
 * 20 minutes is above every ordinary gap. A long build tripping it is fine and
 * expected: the card reports the age and the human judges it, exactly as the
 * schedules card ages `lastTickAt` into "looks stuck". Set 0 to switch the
 * mark off.
 */
 ...
 * this is a cheap clock, not a poll. Clamped below the idle window so a
 ...
 * since the driver refuses a message mid-thought, so a long build or a slow
 * tool call is unreachable by construction. It lands at most once per turn,
 * and it never fires on a scheduled run, where nobody would read the outcome.
 * See lib/idleNudge.ts for the whole argument. Leave it off on an instance
 * running a fleet of long-lived tasks whose waits are legitimate; turn it on
 * where a wedged wait costs more than the turn that would end it.
 */
 ...
 * is a picker's suggestion list, and a slow one must not hold up the app. A
 * server on this machine answers in single-digit milliseconds; the budget
 * only matters for a host that black-holes the connection instead of
 * refusing it.
 */
 ...
 * `--bare`, which never reads the user's login) but it does spawn the CLI
 * once per model, once per CLI version, detached. Turn it off on a box where
 * that CPU is precious, or where `claude` is intentionally absent; the picker
 * then shows the static catalog.
 */
 ...
 * Generous against the time a warm spawn takes, because the cost of waiting is
 ...
 * fail-closed, because the failure it catches would otherwise go unreported: an
 * unknown `-c` override is inert to the codex CLI, so a release that changes
 * the config.toml schema drops the turn back onto the built-in `openai`
 * provider and bills the user's ChatGPT login while the UI still says `local`.
 * Turning this off trades a brief delay on the first turn per endpoint (the
 * verdict is cached against the CLI version that earned it) for that risk.
 */
 ...
 * a shell profile or unit file can't switch turns from the connected
 * subscription login to per-token billing with no warning. Keys saved through
 * the app's own "I have an API key" path are unaffected: they're re-applied
 * from their 0600 files at db init, after the strip.
 */
 ...
 * through startup and recent output without growing unbounded for a dev server
 ...
 * transport the CLI cuts calls off on in resumed sessions (the measurement is
 * in lib/agents/CLAUDE.md), and upstream
 * anthropics/claude-agent-sdk-typescript#436 reports stdio and HTTP servers
 * unaffected. Anything unrecognized falls back to in-process: a typo here must
 * not fail a turn.
 */
 ...
 * remote-aware surface then degrades to the purely local behavior.
 */
 ...
 * reused instead of re-fetched. `gh pr view` is a network round trip per call,
 ...
 * since nobody can see a chip nothing is rendering, and an idle instance
 * shouldn't spawn gh forever.
 */
 ...
 * broke, so this is a tail. It is a knob because the right depth is a property
 * of the repo's CI, not of the app: a linter needs ten lines, a matrix build's
 * stack trace needs a few hundred. Every failing check contributes its own
 * tail, so the prompt grows with the number of red jobs.
 */
 ...
 *
 * Cosmetic and optional, and the only field on the server that distinguishes
 * an instance from several. It is reported by `GET /api/version`, put in the
 ...
 * carries it instead of leaving naming to each client.
 *
 ...
 * Firefox) but rejected by Apple's push service with 403 BadJwtToken, since
 * Apple validates the subject and won't take `localhost`. An instance that
 * wants iOS push must set this to a real `https:` origin or `mailto:` address
 * (or set PUBLIC_BASE_URL to its https origin, which this then adopts).
 */
 ...
 * instead of skipped with no record. For a morning run, arriving at noon and
 * finding it ran is useful; finding it start at 6pm is not. 0 disables
 * catch-up entirely; a schedule can override this with its own catch_up_ms.
 */
 ...
 * arrives quickly; the cap exists because that read happens inside the
 * ticker's single-flight sweep, so a stalled CLI would otherwise wedge every
 * schedule on the instance with no error to show for it.
 */
 ...
 *
 * Every table here except `schedule_runs` grows without bound unless swept:
 * rows are only ever left behind by an FK cascade behind a manual delete. The
 * sweep rides the schedule ticker; these are its knobs.
 *
...
/**
 * Master switch. On by default. Set to off/0/false/no to keep everything
 * forever, the right choice for an instance whose transcripts are a record
 * somebody audits.
 */
...
const days = (name: string, raw: string | undefined, def: number): number => {
  const n = num(name, raw, def);
  // A negative window would delete rows from the future. 0 is meaningful, since
  // it turns that half of the sweep off, so only negatives fall back.
 ...
 * separate, longer window than the task record above, because these belong to
 * the Insights dashboard: /api/insights reads 180 days back and asks for the
 * same width again to compute prior-period deltas, so anything under about
 * 360 days would let a sweep carve a hole in a chart on screen. 400 days
 ...
 * `calandria.db` itself, since freed pages go on the freelist and are reused by
 ...
 *
 * The other half of the retention story: per-task worktrees are a full
 * checkout of the project repo each, so they are the biggest disk-growth
 * vector in the product and the only one measured in gigabytes, not
 * rows.
 */

/**
 * Opt-in, unlike the table prune above. The table windows (180/400 days) are
 * longer than most instances have existed, so defaulting them on changes
 * nothing on an upgrade; a worktree window measured in weeks would start
 * removing checkouts on the first tick after an upgrade nobody asked for. Set
 * to 1/on/true/yes to sweep automatically; the manual path (Settings ->
 * Storage) is unaffected either way.
 */
 ...
 * touched (the sweep reuses prunableTaskIds() instead of owning a predicate),
 ...
 * reclaim is the one that needs telling, since the only remedy there is a
 * human opening Settings -> Storage. In gigabytes; 0 disables the check (and
 ...
 * the box, and a scraper polling every 15s would otherwise walk every
 * `node_modules` on the instance four times a minute for a number that moves
 * in megabytes per hour. A minute is short enough that a disk alert still
 * fires promptly; raise it on an instance carrying many large worktrees. 0
 * measures on every scrape.
 */
 ...
 * key=value` form this app prints) or `json` (one JSON object per line, with
 * `ts`/`level`/`component`/`msg` plus the line's own fields) for an instance
 * whose output is shipped somewhere that parses it.
 *

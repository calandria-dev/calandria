 ...
 * tested without the ambient environment deciding the outcome. Plain CommonJS
 * because server.js needs it synchronously before listen(); it must be
 * COPY'd into the runtime image (Dockerfile), like every other lib/ file
 * server.js reaches at runtime.
 *
 * Two rules:
 *
 ...
 *    Reading it directly makes `npm start` on Fedora try to bind the machine
 *    hostname, binding a LAN interface, breaking http://localhost:3000, or
 *    failing outright when the name does not resolve. There is no reliable
 *    way to tell a user who set HOSTNAME on purpose from a platform that
 *    injected it, so honouring it as a fallback would keep the collision
 *    alive for exactly the users who hit it: the ones who configured nothing.
 *
 * 2. The default is loopback. In local mode the app is unauthenticated and
 *    hands out a shell, so binding every interface publishes it to the whole
 *    subnet. The cross-origin gate (lib/auth/local-origin.mjs) stops hostile
 *    web pages, but it is a header check and cannot defend a published
 *    socket: a LAN client that forges `Host: localhost:3000` walks straight
 *    through it to a shell. Only the bind address closes that. Containers set
 ...
 * ambient-HOSTNAME case) is not in here. */
 ...
 * Hand-rolled instead of lib/env.mjs's readEnv (see the file header): this
 ...
 * ambiguous: HOSTNAME holds something that looks like an intentional bind
 * address while neither CALANDRIA_HOSTNAME nor its deprecated ORCH_HOSTNAME
 * spelling is set. That is very likely an older deployment whose remote
 * access is about to become loopback-only. A bare machine name does not
 * warn: on Fedora that would fire on every boot, which is noise, and that
 * case is the bug this file exists to fix, not a configuration to preserve.
 *
 * @param {Record<string, string|undefined>} [env]

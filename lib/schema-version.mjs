 ...
 * 2 — projects.landing_mode ('merge' | 'pr'): whether a project's work lands by
 *     local merge or by pull request.
 * 3 — projects.auto_reclaim: reclaim a task's worktree and local branch by
 *     itself once its work lands (lib/reclaim.ts).
 * 4 — task_usage.cost_usd widened to NULLable, NULL meaning "this endpoint's
 *     price is unknown" (a custom base URL) as distinct from a measured zero.
 *     The only stamp so far that is not purely additive: the column is REBUILT,
 *     and an older build would both re-impose NOT NULL on the next boot it
 *     migrates and read a NULL back as a number it isn't.
 */

 ...
 *
 * WHY: migrations here are idempotent and almost always additive, so rolling
 * an image tag forward is safe: an older database picks up the new columns on
 * the next boot. Rolling backward is not, since an older build has no way to know
 ...
 *     zero. Not purely additive: the column
 ...
 * 5 - the model_providers table, plus tasks.provider_id,
 *     projects.default_provider_id, schedules.provider_id, schedules.model,
 *     runbooks.provider_id and runbooks.model: a configured model provider is
 *     a row rather than an env blob (lib/providers/).
 * 6 - projects.agent_env and tasks.agent_env dropped. Version 5 converted
 *     every stored blob into a model_providers row and stopped reading the
 *     columns; this stamp removes them. Not additive: an older build re-adds
 *     both columns empty on its next migrate. It then resolves every turn
 *     from a blob that no longer exists, silently returning each project and
 *     task to its agent's cloud login.
 */

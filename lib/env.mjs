 ...
 *
 * None of this app's ~40 knobs were namespaced to the product, so a bare rename
 * would silently fall every existing .env / systemd unit / compose file back to
 * defaults — a task's worktrees would move, the database would move, and the
 * only symptom would be an instance that looks brand new. So the old names keep
 * working indefinitely-ish, and the boot prints ONE line naming the ones still
 * in use (see `deprecatedEnvWarning`, printed by server.js).
 *
 * Precedence is new-wins: CALANDRIA_X if set, else ORCH_X, else undefined. An
 * EMPTY value counts as unset on both sides, deliberately — .env.example ships
 * every key with a blank value and compose forwards blanks for anything the
 * host has not exported, so a present-but-empty CALANDRIA_X must not shadow a
 * real ORCH_X. Callers keep their own `|| default`, so undefined is the shape
 * they already handle.
 *
...
/**
 * Every knob that answers to both names. The list is explicit rather than
 * derived so the boot warning can be complete at boot — the reads themselves
 ...
 *
 * Names NOT here are renamed outright, because both ends are ours: the Codex
 * MCP bridge's CALANDRIA_TASK_ID / _PROJECT_ID / _BASE_URL (injected per turn),
 * and the test-only CALANDRIA_E2E_* / CALANDRIA_TEST_*. The compose-only vars
 * (ORCH_USER/PORT/CPUS/MEM/IMAGE/RUNTIME) are read by docker-compose, never by
 * this process, and are renamed with the rest of the container surface.
 */
...
/* Deprecated names actually READ this process, across every module realm that
 * loaded this file — globalThis for the same reason lib/db-lock.mjs uses it:
 * server.js and the Next bundle each get their own module instance. */
 ...
 * with names from somewhere else would just be wrong.
 *
 * @param {Record<string, string | undefined>} [env]
 * @returns {string[]}
 */

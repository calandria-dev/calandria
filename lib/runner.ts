...
//
// POST /messages calls startTurn() and returns immediately. The turn runs
// here, owned by the server process, persisting every event to SQLite and
// fanning it out over lib/events.ts. A page reload or dropped connection does
// not affect it: any number of GET /messages streams (including zero) can
// watch, and disconnects never touch the turn. Stopping is only ever
// explicit, via lib/abort.ts (/abort route).
...
// Every line this module prints, tagged with this component so
// `CALANDRIA_LOG_FORMAT=json` turns the whole file into parseable output
// without touching a call site (lib/log.mjs).
 ...
 * (the worktree fast-forward notice, if any) is persisted and published first
 * so it reads in order at the top of the turn.
 *
 * `hooks` is what this turn's tool calls may set off outside the turn: today
 * only "a task went terminal, sweep its auto-start dependents". It is passed
 * in rather than resolved here because the sweep lives in lib/autoStart.ts,
 * which launches through this module; importing it back would create an
 * import cycle and would drag the agent SDKs into autoStart's sync
 * route-entry importers (see TurnHooks in lib/agents/types.ts and the
...
export function startTurn(
  ...
  // Registered by task id so the /abort route can find and trip it, and so
  // hasTurn() can report turn liveness to the POST guard and the GET stream's
  // snapshot. Callers that already hold the task's claim (the POST route, the
  // queue drainer's handoff) pass their controller through; anyone else
  // claims here. The claim is atomic (a synchronous check+register), so two
  // concurrent launches can never both start a turn on the same session.
  const abortController = controller ?? claimTurn(task.id);
  if (!abortController) {
    // Defense in depth: the slot is occupied by a live turn. Callers should
    // claim before launching, so this branch should not normally be reached.
  ...
  // Records why this turn is running for its whole life. The permission gate
  ...
  // the user resumed before the reset would be resumed again when it passed,
  // with a "continue" the session didn't need. Announced as an edit because
  // the coarse turn events don't carry the field, and the hero/card chips
  // read it.
  if (task.start_at) {
  ...
  // Detached: nobody awaits this. `run()` guards its own body with
  // try/catch/finally and unregisterTurn runs in that finally, but a throw
  // from the finally itself (e.g. the task row was deleted mid-turn, so
  // updateTask/endSession hits a FOREIGN KEY error) would surface here as an
  ...
  run(task, project, userText, syncNote, abortController, runContext, hooks).catch((err) => {
    ...
    // A crash here means run()'s own finally never reached its settle block.
    // A scheduled run would otherwise leak two ways: the schedule_runs row
    // stuck at claimed/running forever, and the stale RunContext entry left
    // keyed on this task id, which a later ordinary turn on the same task
    // would inherit via interactionDenied() (lib/runContext.ts) and have its
    // own permission prompts auto-deny with no visible explanation. Settle
    // both here, best-effort: this catch is the last line of defense for a
    // detached promise, so nothing inside it may itself throw.
    if (runContext) {
      try {
 ...
 *
 * Without this the message parks in `pending_messages` and waits for the
 * linger to end, which by default is unbounded (BACKGROUND_LINGER_MS = 0). A
 * user watching a `sleep 600` or a `/loop` would be told "queued" and then
 * hear nothing for ten minutes, while the session that could answer them sits
 * idle with an open input. Nothing is in flight during a linger, so the
 * message simply opens the next turn, the same thing the queue drain would
 * eventually do, minus the wait.
 *
 ...
 * lib/turnInput.ts and a `false` return means "queue it instead". Everything
 * after the handoff is the ordinary user-message path: persisted to the
 * transcript, published as a `user` event, so a reload, another tab, and the
 * global lifecycle stream all see exactly what they see for a message that
 * started a turn the normal way. The whole body is synchronous
 * (better-sqlite3, in-memory pub/sub, a synchronous push into the driver's
 * channel), so nothing can interleave between the CLI accepting the message
 * and the transcript recording it.
 */
export function sendToLingeringTurn(taskId: string, text: string): boolean {
  ...
  // /clear could have bumped the generation since. A message persisted under
  // a stale generation renders in the wrong session's transcript.
  ...
  // this one parks behind it. The queue empties on its own, because entering
  // a linger drains the oldest parked message into the same open session
  // (see the background_pending branch in run()).
...
/** Persist and publish a message the live turn just accepted, as the ordinary
 *  user message it is. Shared by both ways one gets in: the user sending
 *  during a linger, and the linger-entry drain of an already-parked
 *  follow-up. */
function recordSentMessage(taskId: string, gen: number, text: string): void {
  // The linger is over the instant the CLI takes the message: a real model
  // turn is starting, so "working in background" would be inaccurate on
  // every surface that reads the row (the activity line, the status dot, the
  // global stream's re-read). Persisted before the publish, like every other
  // state that stream re-reads. `running` stays 1: it never dropped, this
  // turn is the same turn continuing.
  ...
  try {
    ...
    // racing this send): the CLI has the message and the task doesn't exist
    // to show it. Nothing left to do but log it; the turn dies with the task.
 ...
 * worktree, catch a fast-forward-able one up to the base branch, persist and
 * echo the user message, flip running on, and hand off to the detached
 * runner. Shared by the POST /messages resume path and the queue drainer (a
 * dequeued follow-up is always a resume turn). Mirrors the prep the POST
 * route does inline for the very first turn; the initial turn stays in the
...
export async function startResumeTurn(task: Task, project: Project, userText: string, controller?: AbortController, hooks?: TurnHooks): Promise<void> {
  ...
  // message as a queued follow-up, the same outcome as the POST route's guard.
  ...
  try {
    // Worktree isolation is guaranteed here, not by the callers. The POST
    // /messages and lib/autoStart.ts launch paths run this same self-heal
    // before their first turn, but a turn can also reach the runner through
    // the queue drainer in run()'s finally, which only pops a message and
    // calls here. If task.worktree_path were empty at that point, the driver
    // would fall back to project.repo_path and the agent would edit the
    // user's real checkout instead of an isolated worktree, so the check runs
    // unconditionally rather than relying on the launch paths to have already
    // covered it.
    //
    ...
    // return null and fall back to repo_path. Mutates `task` so the sync
    // check below and the runner see the new cwd; runs before the catch-up,
    // which reads worktree_path.
    //
    // A project with no working directory set is skipped rather than
    // isolated: both launch paths refuse that project outright, and
    // ensureWorktree("") would ask git to init a repo in an unknown place.
    //
    // ensureWorktree returning null (non-git/empty repo) is the legitimate
    // fallback above, handled by the `if (wt)` guard. Throwing is different:
    // a stale index.lock from a crashed process, a disk-full git op, a
    // detached HEAD. The queue drain in run()'s finally is the riskiest
    // caller of this self-heal, since it runs on a turn nobody is watching.
    // Swallowing the throw there would launch straight into
    ...
    // event, no transcript line, no banner. So the throw is not caught here:
    // it escapes to this function's own catch below, which unregisters the
    // claim and rethrows. The queue drainer's `.catch` on this same call
    // turns that rejection into a transcript line via publishTurnError, a
    // settled `running: 0`, and a `turn_end`, the same visible-failure path
    // this file uses for every other launch failure. (The POST route ensures
    // the worktree itself before reaching this function, so this branch is
    // its safety net, not its primary path.)
    if (project.repo_path.trim() && (!task.worktree_path || !fs.existsSync(task.worktree_path))) {
      ...
      if (wt) {
        ...
        // Pin the base at the cut (lib/baseBranch.ts): base_sha now comes
        // from that branch, so the task owns the answer from here on. Skipped
        // when ensureWorktree fell back to HEAD (baseBranch ""), since
        // pinning a branch the worktree wasn't actually cut from would be
        // recorded as fact by the merge.
        ...
        // reads as a revert (lib/baseDrift.ts).
        await recordBaseCut({
          taskId: id,
    ...
    // Catch the worktree up to base when it's a clean, zero-conflict
    // fast-forward (no divergent commits, clean tree), so follow-up work
    // isn't built on stale code. Anything riskier is left to the user-driven
    // Sync/Fix banner. A git error must never block the turn, so it just
    // skips the catch-up.
    let syncNote = "";
    if (task.worktree_path && task.work_branch) {
      // The task's own base, not the project's: a task on feature/auth
      // catches up to feature/auth, not to the project's default branch.
 ...
 * DENIED_INTERRUPTED on any open permission card, flipping running and
 * awaiting_input, publishing turn_end). This avoids a bare process.exit(0)
 ...
 * before it exits; that route exists because this function can't be reached
 * by a direct import from the plain-Node entrypoint.
 *
 * abortTurn() deletes a task's registry entry synchronously, so hasTurn()
 * alone can't tell when the unwind is done, only that it's no longer
 ...
 * and nothing should claim a fresh turn while the process is shutting down.
 * This subscribes to it globally before issuing any abort, because a turn
 * whose driver iterator is already at an await-free point could otherwise
 * unwind and publish inside the same synchronous abort() call, before a
 * subscription registered afterward existed to see it.
 *
 * Bounded because a driver stuck on an uninterruptible syscall must not turn
 * a `docker stop` into a hang for the container runtime to SIGKILL anyway; a
 * turn still unwinding at the deadline is left to finish on its own (its
 * finally still runs, this just stops waiting for it) or to be cut off by
 * the process exit that follows.
 */
...
/**
 * Persist and publish a failed-turn line, appending a recovery hint when the
 * failure is one this app knows how to classify:
 *   - the API's context-overflow rejection ("prompt is too long") →
 *     CONTEXT_OVERFLOW_NOTICE, which the UI turns into a one-click "Start
 *     fresh context" (/clear) button;
 ...
 *     for, shown on the gateway card in Settings → Agents;
 *   - a spent usage limit (Claude's 5-hour/weekly subscription cap, an API
 *     429) → USAGE_LIMIT_NOTICE, informational: the recovery is waiting for
 *     the reset, so there is no button;
 *   - an approval-policy block (enterprise-managed Codex downgraded a
 *     "never" policy to one requiring approval that exec mode can't service)
 ...
 *   - a worktree that could not be prepared (a crashed git's stale
 *     index.lock, a registration pointing at a directory that's gone, a full
 *     disk, a detached HEAD) → the classified hint from
 *     lib/worktreeFailure.ts, plus, for the two kinds a repair pass can fix,
 *     WORKTREE_REPAIR_NOTICE, a "Repair worktree" button. This is the path
 *     an unattended launch failure takes (the queue drain below,
 *     lib/autoStart.ts, lib/dispatch.ts), so a scheduled run that fails the
 *     same way every morning states what to do about it.
 * The raw provider text stays visible above the hint either way, so token
 * counts and the actual wording remain legible. The persisted message is the
...
export function publishTurnError(id: string, gen: number, errText: string): void {
  ...
  // (project/task deleted mid-turn): addMessage then hits a FOREIGN KEY
  // error. This function is the error path, so a throw here escapes the
  // runner's catch block and, unhandled on the detached `run()`, would crash
  // the whole server. Degrade gracefully: if the persist fails, still fan
  // out to any live viewer with a best-effort id, and never rethrow.
...
async function run(task: Task, project: Project, userText: string, syncNote: string, abortController: AbortController, runContext?: RunContext, hooks?: TurnHooks): Promise<void> {
  ...
  // suggest_task tool_use ids whose `suggested` event hasn't arrived yet,
  // oldest first. The tool row is created when the call streams; the id of
  // the task it filed is only known when the tool reports back, so the two
  ...
  // follow-up would fail identically) and the whole app told, not just this
  // task.
  ...
  // raised) on LiteLLM's own clock, so the queue is parked for the same
  // reason a dead login parks it, since every follow-up would drain straight
  // into the same rejection. Also raises the instance-wide agent_auth flag,
  // like a dead login, but with a budget-specific reason so the banner and
  // the agent's Settings card don't offer to reconnect a login that never
  // broke.
  ...
  // queue is parked for the same reason, since every follow-up would drain
  ...
  // now would run every follow-up into the same wall.
  let unattendedDeny = false;
  // Set when the pre-turn settings gate refused: the task's own
  // .claude/settings.json changed since it last ran and the change wasn't
  // approved, so this turn never reached the agent. The queue is parked for
  // the same reason as the two above, since every follow-up would raise the
  // identical card, and the task is left flagged, because the way out is a
  // person reading a diff.
  let settingsBlocked = false;
  const startedAt = Date.now();
  // What this turn spent, accumulated from the same `usage` events that
  // write task_usage, so the lifecycle line in the finally can report
  // tokens without re-reading the DB it just wrote. A turn can report usage
  // more than once (each SDK result message carries its own), hence a
  // running sum rather than a last-wins snapshot.
  ...
  // the sum over the others, so this is what stops the lifecycle line from
  // logging a confident "cost_usd: 0" for a turn that may well have cost
  // real money.
  let unpricedTurns = 0;
  // Calandria tool calls the agent CLI answered itself this turn (the
  // `cutOff` tool_result flag, lib/agentToolGuard.mjs). Kept on the turn
  // line so a turn whose tool calls all failed is one grep away, not one
  // per call.
  let toolCutoffs = 0;
  // What the driver has actually reported running, for pricing a gateway
  // turn (lib/gatewayPricing.ts prices by model id, and the gateway's own
  // catalog is the only place that id's rate lives). Seeded from the task's
  // pick and overwritten the moment a "model" event names the resolved id,
  // the same fact resolved_model persists; held here too since a usage
  // event can land before the finally reads it back out of the task row.
  let resolvedModel: string | null = task.model ?? null;
  // Start the idle clock (lib/turnActivity.ts) at launch rather than at the
  // first event: a turn that hangs before the session ever opens is exactly
  // the kind of silence worth reporting, and with no baseline it would never
  // be reported.
  markTurnActivity(id);

  // Turn lifecycle, line 1 of 2: a healthy instance logs the start, and the
  // matching outcome line is emitted in the finally below. Emitted from here
  // rather than from the POST route because this is where every launch path
  // converges: a first turn, a resume, a drained follow-up, a schedule
  // firing, an auto-started dependent.
  //
  // The /metrics counter is incremented from the same two places as the two
  // log lines, so an instance whose graph and whose logs disagree about how
  // many turns ran can't happen.
  countTurnStarted();
  log.info("turn start", {
    task: id,
    project: project.id,
  ...
  // Persist and publish a failed turn's transcript line (with a recovery
  // hint when the failure is classified), and, for a dead login or a spent
  // gateway budget, raise the instance-wide flag every tab reads, published
  // once per outage so a fleet of failing tasks can't spam the banner.
  // Returns which kind of instance-wide failure this was, so the caller
  // records it for the queue decision below. A budget rejection reuses the
  // same flag and relay a dead login does (lib/agents/connections.ts), but
  // with its own reason text, so the banner doesn't tell someone to
  // reconnect a login that never broke.
  ...
  try {
    // The setup below runs inside the try: a throw here (SQLite I/O error,
    // disk full) must still hit the finally, or the turn never unregisters
    // and running never settles, leaving the task showing "running" forever
    // with its queued follow-ups never draining.
    //
    // Records the first-ever turn on this instance (tutorial included;
    // `seeded` tells them apart). The settings flag makes it exactly-once
    // across restarts.
    ...
    // Per-task LiteLLM virtual keys (docs/AGENTS.md, LiteLLM section). Every
    // turn, first or resumed, launched from POST /messages, lib/autoStart.ts,
    // lib/dispatch.ts, or a queue drain, converges here before its driver
    // runs, so this is the one launch-path hook rather than several copies
    // of it. Mutates `task.gateway_key` in place, mirroring the worktree
    // self-heal above. A no-op past the first successful mint, and falls
    // back to the instance key when per-task keys are off, this isn't a
    // gateway task, or the mint fails.
    try {
    ...
    // The pre-turn settings gate. The agent has not started yet, and whatever
    // `.claude/settings.json` says at this instant is what its hooks,
    // permission-allow rules and env are for the whole turn, loaded from
    // disk before canUseTool exists to have an opinion. That file lives in
    ...
    //
    // Hash what the driver says it will load, and if it moved since the
    // version this task last ran under, park here. This uses the same
    // machinery as a tool prompt (same PermissionRequest, same ask registry,
    // same POST /answer route, same transcript row), because the answer
    // means the same thing and a second answering path is a second thing to
    // get wrong. The notice goes first so the transcript reads as a
    // statement followed by a question, and so the fact survives even if the
    ...
    if (changes.length) {
      ...
      // The same settle the driver's permission_decided branch performs,
      // done here because this card was never on the driver's stream:
      // nothing has asked the agent for anything yet.
      ...
      if ("aborted" in waited) {
        ...
        // the finally below settles it as a stopped turn, and the baseline
        // is left alone so the next turn asks again.
      ...
      if ("expired" in waited) {
        ...
        // instantly, an unwatched one after the short grace. Adopting new
        // agent settings is not something an absent human can be taken to
        // have agreed to, so both refuse, and the schedule run settles
        // `failed` off the turnError below rather than reporting a quiet
        // green "ran".
      ...
      // Recorded as allow_once whatever the client sent: with no scope offer
      // on the card, an "allow_always" answer has nothing to remember, and
      // the settled view would claim a rule that doesn't exist.
      settle({ decision: "allow_once" });
    }
    // Both agent SDKs spawn the claude/codex CLI as a plain (non-detached)
    // child tied to this abortController, not to server lifecycle.
    // @openai/codex-sdk passes `signal: <this controller's signal>` straight
    // to Node's `child_process.spawn`, which Node kills on abort natively.
    // @anthropic-ai/claude-agent-sdk's transport schedules its own
    // SIGTERM-then-SIGKILL escalation off the same abortController, and
    ...
    // process.exit(0) reaps Claude children. So `drainActiveTurns()`
    // aborting every live turn on SIGTERM/SIGINT already kills every live
    // agent child for a graceful shutdown; no separate pid-tracking/reap
    // machinery is needed for that path. The one case nothing here covers is
    ...
    for await (const ev of driver.runTurn(task, project, userText, abortController, hooks)) {
      ...
      // event, and clients can upsert by id instead of appending duplicates.
      if (ev.type === "session") {
        sessionId = ev.sessionId;
        opened = true;
        // Session is live: now it's officially started / in progress. And
        // whatever an earlier unattended run left unread is superseded: a
        // turn is underway again, so the row belongs under "In progress"
        // rather than in the ran-clean pile. Cleared here rather than at
        // each launch site because every turn passes through this one
        // point, however it was started (a message, a resume, the
        // deferred-start sweep, the next firing of the same schedule).
        ...
        // and a path it would refuse must not grow a Collaborate button.
        ...
        // A suggest_task call gets a card settled onto it the moment the
        // tool reports what it filed (below). Queue the row so a parallel
        ...
        toolMsgs[ev.id] = { dbId: m.id, data };
        openAsks.add(ev.id);
        // The turn is still live but parked on the user: flag it so the
        // task list / project badges surface "Needs your input" right now,
        // not only once the turn fully ends. Persisted before publishing so
        // reloads and the global /api/events stream (which re-reads the row
        // per event) agree.
        updateTask(id, { awaiting_input: 1 });
        publish(id, { ...ev, msgId: m.id, generation: gen, ts: m.created_at });
        ...
        if (t) {
          ...
          updateMessage(t.dbId, JSON.stringify(t.data));
          // Answered, but only drop the flag once every parked ask is
          // settled; Claude resumes work in the same turn once none are
          // waiting. (A later turn-end re-flags it via the finally block.)
        ...
        // A tool call parked on the user's approval (the canUseTool gate
        // under acceptEdits / plan). Same deal as an ask: persist the
        // request with its id so a reload re-renders an answerable card, and
        // flag the task now, since the turn is live but blocked on a human.
        ...
        if (t && t.data.permission) {
          ...
          updateMessage(t.dbId, JSON.stringify(t.data));
          // Shares openAsks with the question cards: the flag drops only
          // once nothing is waiting on the user, whichever kind it was.
          openAsks.delete(ev.id);
          if (openAsks.size === 0) updateTask(id, { awaiting_input: 0 });
        ...
        // Nothing was ever parked on the user, so openAsks / awaiting_input
        // are left untouched: this is a card that arrives already settled.
        //
        ...
        if (t) {
          ...
          // No tool message to settle onto: the refused call happened
          // inside a subagent (the SDK's agent_id), whose tool_use blocks
          // never surface on this stream. Give it its own card so the
          // refusal is still seen, without the input, which isn't available.
          const request = {
            id: ev.id,
            tool: ev.tool,
        ...
        // slot stays claimed (so Stop, the SIGTERM drain, and message
        // queueing all keep working), and awaiting_input stays 0, since
        // nothing here needs the user, so this state is excluded from the
        // "N need you" pill by construction. The flag is what lets the UI
        // say "working in background" instead of a generic live spinner, and
        // the note is what it's waiting on ("waiting to wake at 12:00"),
        // persisted on the row because every surface that shows it re-reads
        // the row rather than holding the event. Persisted before
        ...
        // "sent at turn end", and the model's turn has ended: a linger is
        // the session holding its input open for work nobody is waiting on.
        // Hand the oldest parked message to it now rather than leaving it
        // until the linger closes, which by default has no deadline at all.
        // One per linger entry, matching the end-of-turn drain popping one:
        // if the work is still running when that turn ends, this fires
        // again on the next entry and the queue empties in order.
        //
        // Does not take startResumeTurn's path: no worktree fast-forward,
        // because the session is already open in that checkout and
        // rewriting files under a running agent is the one thing the sync
        // is careful never to do. Generation-guarded like every other write
        // in this loop.
        ...
        // continuation turn is about to stream with no user message behind
        // it. Persist the CLI's own summary as the system line that
        // explains the unprompted output; without it the transcript shows
        // the model talking to nobody.
        ...
        // The suggestion is already committed; what happens here is only
        // about where the user sees it. It settles onto the suggest_task
        // tool row the call created, the same move an already-decided
        // permission card makes above, so the proposal is reviewable in the
        // session that made it instead of only in a tray the user has to go
        // find.
        //
        // Nothing but the two ids is persisted: the card re-reads the task
        // on every render, so a transcript reloaded next week shows what the
        // row is now (started, accepted, withdrawn, deleted) rather than the
        // offer that was true the moment the tool ran. A driver that reports
        // no task id, or a suggestion with no call to settle onto (the
        // mock's directive path before it grew a tool row), falls through to
        // a bare publish; the tray still refreshes.
        // Only a report that names its task consumes a queued call: a
        // driver that omits the id has no card to place, and popping the
        // queue for it would offset every later suggestion onto the wrong
        // row.
        ...
        // either, since the client adds a usage event to the task's running
        // total, and the full report that supersedes this one would then be
        ...
        // A report with nothing in it is not a report. Every resumed
        // session's first result message carries all zeros, which would
        // otherwise write a cost-0/tokens-0 row seconds into every resume:
        // noise in a per-turn ledger, and a "turn" the unpriced count and
        // the turn tally both saw. It must not supersede the provisional
        // accumulator either: an empty report says nothing about the
        // requests already made.
      } else if (ev.type === "usage") {
        // The segment's own totals cover every request this turn made since
        // the last full report, so whatever was accumulated per request is
        // now double, and is dropped.
        ...
        // estimate prices an unknown id at the CLI-default family. "Not
        // vendor spend" splits further, and folding the cases together is
        // decided once, here, before the ledger, the running total and the
        // live chip all read it:
        //
        //  - a local model server (Ollama, LM Studio, anything on this
        //    machine or this network) genuinely bills nothing, so 0 is a
        //    measurement;
        //  - a custom base URL is free text plus an optional token, as
        //    likely to be OpenRouter, Together, Fireworks or a Bedrock proxy
        //    as anything free. Writing 0 there under-reports a real bill and
        //    writing the driver's figure over-reports a catalog that isn't
        //    the one being charged. So it is recorded as unknown (null) and
        ...
        //    probe, falling back to the provider's pinned model when the
        ...
        // The wire event stays TurnUsage-shaped, and the client adds it to
        // the task's running total, so an unpriced turn contributes 0 there
        // for the same reason SUM() skips its NULL, with `unpriced`
        // alongside so the client bumps the row's unpriced_turns and marks
        // the total as a floor immediately, rather than showing a stalled
        ...
        // A soft error emitted mid-stream (e.g. "Run ended: ..."). Marks the
        // turn failed and publishes the persisted form, so live viewers and
        // snapshot replays render the identical line (with a recovery hint
    ...
    // listening when this fires, so the transcript must carry it.
    ...
    // so nothing can interleave with it: the registry slot is either handed
    // off or released below, never left in a half-state a POST could race.
    //
    // A Stop deletes the registry entry immediately (hasTurn goes false), so
    // a new POST can claim the slot and start a successor turn while this
    // one is still unwinding. When that has happened, the successor owns
    // the task row, the pending queue, and turn_end; settling any of them
    // here would clobber a live turn's state (running flipped off, its
    // queued follow-ups eaten).
    const superseded = hasTurn(id) && !ownsTurn(id, abortController);
    // Any card still open never got a decision: the turn died (Stop, a
    // crash, a driver error) with the user parked on it. Settle it here or
    // it renders as answerable forever, and answering it would resolve
    // nothing. Both kinds ride openAsks and both need the backstop: an
    // unanswered question is the same problem as an undecided permission
    // card, so `ask` gets a dismissal marker rather than being left blank,
    // never a fabricated answer. Best-effort: this is the finally, and the
    // task row may already be gone.
    ...
    // Guard against a generation boundary crossed mid-turn (a /clear while
    // this turn was live). /clear ends this generation and resets the task
    // row (session_id=null, started=0, running=0) for a fresh context.
    // Writing sessionId back here would resurrect the session /clear just
    // nulled and re-arm running/awaiting_input, defeating /clear and
    // double-injecting the old context on the next send. So the task row is
    // only settled when it's still on the generation this turn actually ran
    // in.
    const current = getTask(id);
    const generationAdvanced = !current || current.generation !== gen;
    // If the session never opened, keep the task retryable (started stays
    // 0). A turn that actually ran and ended mid-task, whether it finished
    // on its own or was Stopped, is now waiting on the user, so it flags
    // awaiting_input (cleared on the next send / done), leaving it cleanly
    // resumable.
    //
    // A scheduled turn that finished cleanly is not waiting on anybody:
    // nobody asked for it, and awaiting_input feeds the shared NEEDS_YOU
    // predicate behind the "N need you" pill. Left at 1 it would park a
    // permanent, unanswerable item there every morning, which is how a user
    // learns to ignore the pill. Success is quiet; a scheduled turn that
    // failed still raises its hand like any other.
    // An interaction can be auto-denied by a path that doesn't emit a
    // permission event: the Codex ask_user bridge (lib/agentTools.ts),
    // which records it on the context instead. Folded in here, before
    // anything below reads the flag, so "the turn was cut short because
    // nobody was there" has exactly one meaning regardless of which agent
    // hit it.
    if (runContext?.deniedInteractions) unattendedDeny = true;
    // Success is quiet, but only actual success. A turn whose tool calls (or
    // questions) were auto-denied because nobody is watching did not do the
    // job it was scheduled for; it stopped partway with the work half done.
    // Reporting that as a quiet green "ran" is precisely the silent skip
    // this feature exists to make impossible, so it raises its hand like
    // any other failure.
    ...
    // accumulator. Without this, a turn stopped mid-segment records 0
    // tokens and $0, because the ledger otherwise only reads result
    // messages. No price is recorded with it: the per-request source
    // carries tokens alone, so the row is unpriced (null) rather than a
    // confident $0, except against an endpoint that genuinely bills
    // nothing, where 0 is a measurement, exactly as the live branch above
    // decides it. Best-effort like the ask settle: this is the finally, and
    ...
    if (!isEmptyUsage(provisional)) {
      try {
        ...
        // place. A gateway turn still gets a real estimate here: the
        // accumulator holds exactly the tokens a full report would have
        // priced.
        const gatewayEstimate =
    ...
    // Turn lifecycle, line 2 of 2: what happened, how long it took, what it
    // cost. The outcome ladder is the same one the schedule-run settle below
    // uses, since a run recorded `failed` in the ledger and logged `ok`
    // would be worse than no line at all, with `interrupted` meaning the
    // agent session never opened, so the turn produced nothing. Tokens come
    // from the accumulator, not a task_usage read: a driver that reports no
    // usage at all logs zeros, which is the truth about this turn rather
    // than the task's running total. A turn that died before its result
    // message doesn't log zeros: the flush above put its per-request tokens
    // in the accumulator first, with no cost, so the line reports them
    ...
    // The counter takes the same word: TurnOutcome is that ladder as a type,
    // so a fifth outcome added here has to be given a series too rather
    // than landing in no bucket at all.
    ...
    log[outcome === "failed" ? "error" : outcome === "interrupted" ? "warn" : "info"](`turn ${outcome}`, {
      ...
      // Included only when they say something: `superseded` means a Stop
      // released this turn's slot and a successor claimed it, so this line
      // describes a turn that settled nothing; `error` is the reason the
    ...
    if (!generationAdvanced && !superseded) {
      // background_pending settles with running: however the linger ended
      // (all work done, expiry, Stop), the session that owned the work is
      // gone.
      //
      // A clean scheduled run rests on `unread_run_at`, the mark that says
      // "this ran, on its own, and nobody has looked at it yet". It sits
      // outside the NEEDS_YOU predicate, so the "N need you" pill never
      // gains a daily item nobody can answer, but it is still a state with a
      // way out of it: the board draws these in their own group and
      // acknowledging one is an ordinary status write. Without it the task
      // would sit at running=0 / awaiting_input=0 / status=in_progress,
    ...
    // Exact spend reconciliation (docs/AGENTS.md, LiteLLM section): GET
    // /key/info against the task's own minted key, recording the delta as a
    // task_usage correction that replaces the live per-turn estimate with
    // LiteLLM's own ledger. Fire-and-forget, not awaited: this whole block
    // is the synchronous settle the note at its top requires nothing
    // interleave with, and an HTTP round trip here would do exactly that.
    ...
    // Settle the schedule run from here, because this is the only place
    // that knows the outcome: whether a session opened, whether it errored,
    // whether it was stopped. Polling task.running from outside cannot
    // reconstruct it.
    if (runContext?.scheduleRunId) {
    ...
    // A turn that actually ran is the strongest possible proof the agent's
    // login (and, for a gateway task, its budget) works, stronger than
    // `claude auth status`, since it exercised the same path a real turn
    // takes. So clear any broken-connection flag left by an earlier failure
    // and tell every tab, which is how the banner comes down after the user
    // reconnects in a different window (or the credential refreshed, or the
    // budget reset). Publishes only when the flag was actually set.
    if (!turnError && opened && clearAgentAuthBroken(task.agent)) {
      publish(id, { type: "agent_auth", agent: task.agent, broken: false, reason: null });
    }

    // A Stop, or a /clear that advanced the generation out from under this
    // turn, discards the parked queue: those follow-ups were lined up
    // behind the train of thought the user just interrupted (or the context
    // they just cleared), so running them now would be surprising and would
    // leak old-generation work into the new one. Otherwise dequeue the
    // oldest follow-up and run it as the next turn, continuing the session
    // without a gap.
    let continued = false;
    if (superseded) {
      // The successor turn owns the queue and will emit its own turn_end;
      // this registry entry is long gone (the Stop deleted it), so there is
      ...
      // The login (or the gateway budget, or the quota, or the absent
      // human, or an unapproved settings change) is what failed, not the
      // work: draining now would run each follow-up straight into the same
      // authentication error / spent budget / spent limit / unanswerable
      // permission prompt / identical settings card, emptying the queue and
      // stacking identical walls of red for messages that never actually
      // ran. Leave them parked (they're rows in pending_messages, so they
      // survive a reload and still render as queued bubbles) and say so
      ...
      if (parked) {
        ...
        // Best-effort: the task row can be gone by now (deleted mid-turn),
        // and a FOREIGN KEY throw here would escape the detached run()'s
        // finally.
        try {
          const m = addMessage(id, gen, "system", note);
      ...
      // Hand the occupancy slot to the follow-up first, an atomic swap of
      // this controller for a fresh one, so hasTurn never reads false
      // between this turn and the next. An unregister-then-launch sequence
      // would leave a window where a POST could start a parallel turn
      ...
      if (nextController) {
        ...
        // Re-read the project at dequeue time, not the snapshot captured at
        // the start of the turn being finished. The base branch, repo_path,
        // or context may have changed while this turn ran; the dequeued
        // follow-up fast-forwards against its resolved base branch /
        // repo_path and seeds its system prompt from project.context, so a
        // stale snapshot would sync it to the wrong base and run it with
        // outdated context.
        const freshProject = fresh ? getProject(fresh.project_id) : undefined;
        if (next && fresh && freshProject) {
          // Drop its "queued" bubble: startResumeTurn re-echoes it as a
          // normal user message. running stays on across the handoff, so
          // turn_end is not published here; the next turn's own finally
          // will.
          publish(id, { type: "dequeued", msgId: next.id });
          continued = true;
          // The follow-up writes into the same worktree the merge/sync
          // routes rewrite with multi-second git ops, so it launches under
          // the same per-task lock those routes hold, then re-checks the
          // world once the lock is acquired, since the wait can be long.
          // The handoff slot above stays claimed the whole time, so no POST
          // can start a parallel turn while this waits for the lock.
          void withTaskLock(id, async () => {
            const cur = getTask(id);
            // Task deleted or /clear'd while waiting: a cleared generation
            // discards its queue, and this popped message belongs to the
            // old one. (Their abortTurn already released the handoff slot;
            // the unregister is a defensive no-op then.)
            if (!cur || cur.generation !== gen) {
              unregisterTurn(id, nextController);
              return;
            }
            // Re-read the project too: it was already refreshed at dequeue
            // time (see freshProject above), but the lock wait can be long,
            // and a merge may have run, or the project may have been
            // deleted, in between.
            ...
            if (!ownsTurn(id, nextController)) {
              // A Stop landed while waiting on the lock: it tripped and
              // released the handoff slot (and a successor turn may have
              // claimed it since). Stop discards queued follow-ups, so drop
              // this one, since it was parked before the press. With no
              // successor, settle the UI with the turn_end the handoff had
              // deferred; a live successor emits its own.
              if (!hasTurn(id)) publish(id, { type: "turn_end" });
              return;
            }
            // Same hooks the turn being finished ran under: a drained
            // follow-up is the same session continuing, so its tool calls
            // must reach the same launcher (nobody re-supplies them from
            // outside).
            await startResumeTurn(cur, curProject, next.content, nextController, hooks);
          }).catch((err) => {
            // Failsafe: if launching the queued turn fails, release its
            // slot, surface the error, and settle the task so it doesn't
            // hang in a running-but-dead state.
            unregisterTurn(id, nextController);
            // publishTurnError persists and publishes without ever
            // rethrowing: a plain addMessage here could hit a FOREIGN KEY
            // error (task deleted while waiting on the lock) and, thrown
            // from inside this .catch, become an unhandled rejection that
            // crashes the server.
          ...
          if (next && fresh && !freshProject) {
            ...
            // / context left, the follow-up can't be safely synced or run.
            // Drop its "queued" bubble, surface the cancellation, and fall
            // through to the turn_end below (continued stays false) leaving
            // the task settled.
            publish(id, { type: "dequeued", msgId: next.id });
            publishTurnError(id, gen, "Project was deleted; queued follow-up cancelled.");
          }
          // Nothing queued, or the task/project row is gone: free the slot
    ...
    if (!continued) {
      // Release occupancy only now, at the very end of this synchronous
      // block: a no-op if a Stop already deleted the entry or a handoff
      // replaced it.
      unregisterTurn(id, abortController);
      // The idle clock only describes a live turn, so it retires with the
      // slot. Not on the handoff/superseded paths: the successor turn is
      // the same session continuing, and it re-stamps on entry anyway.
      forgetTurnActivity(id);
      // Emitted after the task row settles, so a client refreshing on
      // turn_end reads the final running/awaiting_input state. Skipped when
      // a queued follow-up or a successor turn is taking over; that turn
      // will emit its own turn_end.
      publish(id, { type: "turn_end" });
      // A PR this session opened by hand, because create_pr was cut off
      // before it reached the app (lib/agents/CLAUDE.md), is linked here.
      // Detached and after turn_end, since it may spend a gh subprocess on
      // github.com and the turn is over; the chip arrives on the
      // task_edited it publishes, the way every other background PR read
      // reaches the client. Screened down to almost nothing on the ordinary
      // task (see adoptExistingPr).

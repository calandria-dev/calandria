...
- Under every mode but `bypassPermissions`, the SDK's `canUseTool` is a real gate
  ...
  (`permission_rules`), then a permission card that parks the turn on the user via `lib/asks.ts`
  and `/answer`. Every non-answer path denies (Stop, expiry, unwatched turn, unparseable answer),
  and an unattended auto-deny parks the pending queue.

...
## Fidelity
**High-fidelity.** Colors, typography, spacing, and copy are final. Recreate pixel-perfectly. The one intentionally mocked area: session data is fake; wire to real state.
...
- **Logomark**: `assets/logo.svg`, a 3×3 isometric lattice of control rods, radial opacity fade from raised center rod (opacities 1 / .55 / .3), each rod standing in a thin open "channel ring". Single color via `currentColor`; tint with CSS. viewBox `4.78 6.6 14.45 17.23`. No raster.
...
- **Wordmark**: `Calandria`, set in Spectral 500, title case (never all-caps in the lockup), `letter-spacing:.005em`.
...
- `--font-body`: **Source Sans 3** (system-ui fallback), for all dense UI: rows, chips, labels, controls, card titles.
- `--font-mono`: **JetBrains Mono** (ui-monospace fallback), for code, terminal, diffs, metadata. Default.

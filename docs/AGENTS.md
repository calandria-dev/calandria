...
Calandria's own background jobs run with your MCP servers, plugins, skills, and hooks switched
off: a `/clear` handoff note, a project recap, a "Refresh with AI" context draft and a "Refresh
tag" plan check are internal transformations, not sessions you're sitting in. They still read
`~/.claude/settings.json`, since that's also where a Bedrock/Vertex/proxy setup keeps its `env`
block and `apiKeyHelper`, so they authenticate the same way your ordinary turns do. The two
repo-reading jobs additionally load the repository's `CLAUDE.md`, and can read, search, and list
files, but not run commands or write anything. Which model each of them runs on is the two-tier
...
| `CODEX_INHERIT_MCP` | unset (off) | Set to `1` to mount the MCP servers from your own `~/.codex/config.toml`. Worthwhile if you've set `default_tools_approval_mode = "approve"` on those servers. |

**Permission modes.** The non-interactive CLI can't pause an active turn for a command-approval
prompt, so Calandria offers Codex's own sandbox modes instead of a mid-turn approval mode:

| Mode | What it does |
|-|-|
| **bypassPermissions** | Codex's **workspace-write** sandbox: writable, never asks. |
| **plan** | Codex's **read-only** sandbox: no writes, no network. |

**What is not supported.**

- No mode maps to an interactive per-call approval prompt (Claude Code's `acceptEdits` and
  `default`): the non-interactive CLI has nothing to pause on.
- No equivalent of Claude's permission cards for a refusal Codex makes on its own.
- MCP servers from `~/.codex/config.toml` are not mounted by default (see `CODEX_INHERIT_MCP`
  above): an inherited server's tools would be offered to the model with every call returning
  `user cancelled MCP tool call`, so Calandria unmounts them instead of leaving tools that can't
  work.
...
- The context-window gauge is an estimate, marked `≈`. Claude's stream reports each model
  request's usage, so a Claude task's gauge shows the window's actual contents as of the latest
  request. `codex exec` reports only the thread's running totals on `turn.completed`, so a Codex
  task's gauge is derived from its last turn's usage report, and a turn spans many requests (every
  tool call re-reads the whole context), so a tool-heavy turn over-reads.

Each agent's card in **Settings → Agents** states which side of the MCP-inheritance gap it's on.
...
mounted server also carries `default_tools_approval_mode: "approve"`, which auto-approves every
one of its tools for the task the moment it mounts. That is only offered under the
bypass-equivalent permission mode; a task set to `plan` mounts none of them, the same reason
`codex/mcp.ts` unmounts the user's own inherited servers under a mode with nothing to call them.
Settings → Agents states the gate on Codex's card.

...
| No process groups; `detached` means "new console"; no `ps` | `lib/processTree.ts`: `taskkill /pid <pid> /T /F` for the tree, signal 0 for liveness (Node documents it as the existence test on Windows too, and it costs nothing, where `tasklist` measured 9.5-10.5s per call on a real desktop), a `Win32_Process` CommandLine lookup for the recycled-pid guard, and `detached` requested only where groups exist. The guard is tri-state: a lookup that cannot answer is retried and then declined rather than reported as a mismatch (#324). A `dev_command` is a `cmd.exe` command line ([SERVICES.md](SERVICES.md#windows-command-syntax)) |

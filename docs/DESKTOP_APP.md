...
| The real server boots under the shell's supervisor | `node desktop/test-real-boot.js`: **919 ms** to `/api/version`, on ports 3002/3003 (stepped past the live instance), app HTML served, `/pty` proxied to the sidecar we chose, `ORCH_DB_DIR` honoured, both sidecars drained on SIGTERM with no SIGKILL. |

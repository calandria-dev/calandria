...
to the server, not an agent turn or browser tab, so they keep running after a turn ends and
their state and logs stay available when you reconnect.
...
that was running is restarted when Calandria boots.
...
PID. A port conflict with an unmanaged process is reported as a readable error, so a service
never crash-loops on a busy port. Log retention is bounded by `CALANDRIA_SERVICE_LOG_LINES`
(1,500 lines by default).
...
- single quotes: `cmd.exe` only understands double quotes; `'…'` passes through literally.
- `~`: use `%USERPROFILE%`.
...
`pwsh -NoProfile -Command "…"` or `bash -lc "…"` with Git Bash installed. Calandria doesn't
...
`ORCH_PUBLIC_HOST` is also injected into every managed service indefinitely, so an existing
config referencing the old name keeps working unchanged.

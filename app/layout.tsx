...
export const metadata: Metadata = {
  ...
  // URL; leave metadataBase unset instead of hardcoding a domain this instance
  ...
  // Named instances put their name first: the document title is what a
  // browser puts in the tab, and a user running two instances needs the tabs
  // to read differently. Unnamed (the default) is unchanged.
  ...
  // under the status bar, which the titlebar handles via viewport-fit=cover
  // and safe-area-inset padding (globals.css).
  appleWebApp: {
    capable: true,
...
// instead of baked in at build time, so a prebuilt image stays relocatable via
// env only.
    ...
    <html lang="en" data-theme="cherenkov-dark" data-mode="dark" className={fontVariables}>
      <head>
        ...
        {/* Hand the instance's public origin to client code (Terminal builds its
            ...
            a stored override against (lib/agentEnv.ts): the client classifies
            an override the same way the server does. The address itself is not
            a secret; the key that goes with it never leaves the server. Whether
            per-task keys are available (CALANDRIA_LITELLM_ADMIN_KEY set) rides
            GET /api/agents' gateway_keys_enabled instead of a window global,

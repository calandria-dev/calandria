...
// or the clock did) and does not move the next occurrence.
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // already owns this instant. Anything else THROWS instead of reporting the
    // same "already claimed" (see claimRun), so it lands below with the actual
    // reason instead of a 409 that sends the user looking for a run that does
    // not exist.

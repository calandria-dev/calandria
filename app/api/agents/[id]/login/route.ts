...
// DELETE abandons it. Resolving the driver by [id] means a new agent needs no
// new route, only a driver that registers an auth surface.
//
// This persists per-agent connection state (lib/agents/connections.ts),
// separate from the single onboarding record app/api/claude/login uses: the
// wizard's required first step stays Claude-specific, while this powers the
// "connect another agent" cards and the task-creation gating for every

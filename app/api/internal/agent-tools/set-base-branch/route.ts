...
//                omitted means "my own row", which is the common case: a session
//                retargeting ITSELF mid-turn is what this tool is for.
//
...
// so a user's retarget and an agent's cannot mean two different things.
//
...
export async function POST(req: NextRequest) {
  ...
  // either named a branch that can't be used or a row that may not be
  // retargeted. `error` is what the bridge shows the agent as the tool's
  // failure text — which is what tells it how to retry.

...
// subprocesses, so it is deliberately NOT fetched when a task is merely
// selected: the button asks for it on the click that arms it.
...
// rather than trusting the preview this client rendered — so work that appeared
// after the preview is refused rather than swept up.
//
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return jsonGuard(`reclaim ${id}`, async () => {
    ...
    // unwraps that one key, so a refusal reads as its own sentence rather than
    // as a raw JSON blob in a banner.

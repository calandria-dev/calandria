...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // where it came from. It deliberately does NOT go through the dispatcher —
  // there is no turn to launch, and the dispatcher's whole preflight exists to
  // avoid minting a task that cannot run.
  if (body.start === false) {
    const task = createTask({
      project_id: runbook.project_id,
      title,
      // The composed prompt becomes the brief, because nothing is going to send
      // it as a turn — without this the extras typed for this run would be lost

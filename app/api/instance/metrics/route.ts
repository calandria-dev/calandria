 ...
 * footprint, schedule-run outcomes. lib/metrics.ts holds the series and the
 * reasoning behind each one; this file is only the transport.
 *
 ...
 * CALANDRIA_FLEET_TOKEN, which is the point of that token, one secret for a
 * dashboard polling every box. In no-login local mode the ordinary origin rules
 * apply and a loopback scrape needs nothing.
 *
 ...
 *
 * `Cache-Control: no-store` because a scrape that a proxy answered from cache
 * is a flat line, which reads as a healthy idle instance rather than as a
 * missing one.
 */
export async function GET() {
  try {
    ...
    // Deliberately not the JSON shape lib/apiGuard.ts produces: the caller here
    // is a scraper that asked for text, and Next's HTML error page would be

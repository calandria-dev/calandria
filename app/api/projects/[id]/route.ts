...
async function withDiffStats(project: Project, tasks: TaskWithUsage[]) {
  ...
  // expired ones once the map gets large instead of checking on every call.
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // that disagree.
  // idle_since rides this read instead of listTasks, which is DB-only: the
  // mark is in-memory turn state (lib/turnActivity.ts), and this is the
  // endpoint the shell builds its task rows from. Without it, a reload during

 ...
 * saved yet, since the project settings dialog needs suggestions to appear
 * while the URL is being typed, before there is a stored override to read.
 * It is not a wider reach than the caller already has: the same person can
 * save any URL to the project and have every turn talk to it, and all this
 * does with the answer is list model ids.
 *
 * A cloud project answers with an empty, reachable-false body instead of a
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // would 404. This branches ahead of that probe instead of falling through
  // to it. `?agent=` says which driver's picker this is for, since the
  // gateway's fit filter differs by driver (lib/gatewayModels.ts);
  // unset/unrecognized falls back to the project's own default agent.
  //
  // Checked against `base`, the URL actually being probed, since `?base_url=`
  // lets the picker ask about a URL that isn't saved yet: typing the

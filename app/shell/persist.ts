...
// The selection half of a persisted blob — all `landingSelection` reads. Nullable
// as well as optional: the live shell state is `string | null`, and a blob written
// before boot landed (see `selectionToPersist`) round-trips those nulls.
...
// rather than by reading a fetch callback.
...
// them would erase the very value the next start reads back — which is what made
// a restart land on the first project. Before then, re-write what is already

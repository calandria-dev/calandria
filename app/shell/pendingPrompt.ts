 ...
 * SessionView lifts these rows out of the flow into a dock pinned below the
 * transcript, which is what this picks out.
 *
 * Liveness is the hard part, because "the row has no answer" is not the same
 * as "the turn is waiting". Nothing backfills an ask row that a Stop or a
 * crash tore down (`lib/agentTools.ts` creates the card; `recoverFromCrash`
 * only backfills PERMISSION rows), and `tasks.awaiting_input` is zeroed by the
 * next turn start regardless. A dead question can sit unanswered in the
 * transcript forever, so pinning one would put a card nobody can usefully
 * answer permanently on screen. Three checks, none of which can be dropped:
 *
 ...
 *   already typed past counts as answered in prose or abandoned either way.
 *
 * Returns ids in transcript order, as a LIST because a parallel tool batch can
 * park on several cards at once, and docking one while its siblings stayed
 * inline would be a worse version of the bug being fixed.
 */
 ...
 * `awaiting_input` counts too, but not once the task itself is terminal, since
 * that is where "somebody owes an answer" stops being true.
 */

...
// The titlebar "N need you" dropdown. Lists every task that needs the user across
// all active projects: parked on a question, or sitting on an open PR whose checks
// are red (lib/store.ts's NEEDS_YOU predicate, both arms). Each row shows its
// project, title, and why it's here.
// Picking a row jumps straight to that task in its project. Rows are fetched fresh
// on open so the ages and membership stay current; the longest-waiting task sits
// at the top (server orders by waiting_since ASC).
          ...
          rows.map((r) => (
            ...
            >
              ...
              <span className="ny-text">
                ...
                {/* A red PR has no "waiting for" age: the row only knows when it
                    last asked GitHub, not when the build broke, so it names its
                    PR instead of inventing a duration. */}

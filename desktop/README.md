...
| `instances.js` | The saved instance list (which server the window attaches to) and the version handshake. `local` is the pair of sidecars `supervisor.js` spawns; a `url` entry is an origin the shell attaches to over the network; an `ssh` entry is one reached through a port forward (`ssh-tunnel.js`). Each in its own persistent Electron partition so an Access cookie cannot bleed between them. Holds the file at `~/.config/calandria/instances.json` (`CALANDRIA_INSTANCES_FILE` overrides) and repairs a hand-edited one instead of refusing to launch over it. No `require("electron")`. |

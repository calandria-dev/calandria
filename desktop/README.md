...
| `instances.js` | The saved instance list — which server the window attaches to — and the version handshake. `local` is the pair of sidecars `supervisor.js` spawns; a `url` entry is an origin the shell attaches to over the network, each in its own persistent Electron partition so an Access cookie cannot bleed between them. Holds the file at `~/.config/calandria/instances.json` (`CALANDRIA_INSTANCES_FILE` overrides) and repairs a hand-edited one rather than refusing to launch over it. **No `require("electron")`**, like its neighbours. See [`docs/DESKTOP_APP.md`](../docs/DESKTOP_APP.md) §8. |
| `instances.html` | The Add / Manage instances dialog. A static document whose CSP forbids its own scripts, like `loading.html`; `main.js` injects the behaviour with `executeJavaScript`, which is how the dialog gets a form and a list without shipping a preload into every page the window later loads. |

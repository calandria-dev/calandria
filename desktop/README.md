...
The bundled binary is authoritative once it exists. The supervisor retries a
failed version probe three times, logs the operating-system error from each
failure, and stops startup if every attempt fails. It falls through to a PATH
Node only when no bundled binary is present. This prevents a transient first-run
execution failure from silently changing the runtime used for the packaged
payload.

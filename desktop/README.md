...
| `window-state.js` | Where the window was last left: size, position, maximized and full-screen, in `~/.config/calandria/window-state.json` (`CALANDRIA_WINDOW_STATE_FILE` overrides). Electron remembers nothing between two `BrowserWindow`s, and this shell builds a new one on every instance switch that crosses a session partition, so a window built from literals reopened at those literals every time. `fitToWorkAreas` is the half worth reading: a saved rectangle can name a monitor that has since been unplugged, and restoring it unchecked opens the window off-screen. Electron-free, so the geometry math is tested under plain node; `main.js` passes the display work areas in. |
...
| Path | What it is |
|-|-|
| `resources/app.asar` | The Electron shell: `main.js`, `supervisor.js`, `notifier.js`, `tray-residency.js`, `window-state.js`, `loading.html`. |
...
`.github/workflows/release-desktop.yml` prebuilds the release on the generated
release-please PR. Three runners build the signed installers, update feeds and
blockmaps, then save them with manifests, checksums and signing notes as
90-day workflow artifacts. The PR job uses `pull_request_target`, validates the
exact release branch and changed files, and invokes electron-builder with
`--publish never`.

When release-please merges the PR and creates the `v*` tag, tag-only promotion
finds the merged PR and its unexpired artifact run. It verifies the version,
PR head, tag tree, manifest metadata, byte counts and SHA-256 hashes, then uses
`gh release upload` for every already-generated file, including every feed and
blockmap. Desktop promotion does not wait for a duplicate push `Test` run.
`publish-image.yml` keeps its `require-green-test-run` gate for image releases.
...
A manual run with `publish=true` retries tag promotion. A manual run with
`publish=false` is a dry run that builds, signs, notarizes, staples and runs
every assertion without publishing. It stores the installers as downloadable
workflow artifacts.
...
and failing the job if a name matches zero or more than one asset. The NSIS
installer now uses the explicit `Calandria-Setup-<version>.exe` name, and the
release gate checks that `latest.yml` references files present in the handoff.
This catches feed and asset filename drift before a release is published.

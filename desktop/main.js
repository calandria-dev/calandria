...
function main() {
  ...
  app.on("before-quit", async (event) => {
    ...
    // showing the provider, which is the correct thing for it to show: the app
    // that asked for the sign-in is going away.
    cancelSignIn();
...
async function attachOrigin(inst, origin, seq) {
  ...
  // An identity provider standing in front of an instance with NO configured
  // sign-in is still handled the old way: load the page and let the user
  // complete it in the window. That path is not dead code and must not be
  // broken — a Cloudflare Access one-time PIN, a password form, an OTP are all
  // things this window can finish, and the instances doing that today would be
  // regressed by an app that refused to render them. What it cannot finish is
  // anything that hands off to the system browser, which is why `did-navigate`
  // puts an offer of the native flow on top of that page (`maybeOfferNativeSignIn`).
 ...
 *
 * SILENTLY FIRST. A stored refresh token is the common case on every launch
 * after the first, and renewing it is a round trip to the token endpoint that
 * the user never sees. The screen is only for when that is not possible: no
 * credential, a refresh token the provider has revoked, or a first run.
 *
 * The loop is because each answer leads back to the same question. A sign-in
 * that failed at the provider, a credential that turned out not to be enough
 * for whatever is out front, a config that needed fixing — all of them end with
 * the user looking at this screen again, with the reason on it, rather than at
 * a modal that took the context away to say so.
 */
async function resolveSignIn(inst, origin, seq) {
  ...
  for (;;) {
    ...
    if (answer === "configure") {
      ...
      if (seq !== attachSeq) return "abandoned";
      // Re-attaching rather than continuing with the row this call closed over:
    ...
    // the proxy wants a header this one is not being sent as. Saying which of
    // those it is needs the proxy's log, so say what IS known.
    error =
 ...
 * page goes away underneath, for `showAttachFailure`'s reason: a destroyed
 * window means a switch or a quit is already happening and the caller's seq
...
async function showSignInPrompt(inst, { error = "" } = {}) {
  ...
  const buttons = [];
  // A `header` instance with nothing stored has no flow to run, so the primary
  // action is the dialog that takes the credential rather than a browser that
 ...
 *
 * Only reached by an instance with NO configured sign-in, which is the one case
 * where a third-party login page is still rendered in this window. It usually
 * works, and when it does not the failure is silent and baffling: the page
 * hands off to the system browser for a passkey, the ceremony succeeds there,
 * and this window sits at the same screen with a cookie jar that never heard
 * about it. There is no event that says so, so the offer is made up front,
 * whenever the window ends up somewhere that is not the instance.
 *
 * Dismissible, and never on the app's own pages — it is advice about a login,
 * not a banner to live under.
 */
...
function startSubscriber(inst, origin) {
  ...
  sub.events = new AppEvents({
    ...
    // `credentials: "include"` is what actually attaches CF_Authorization, and
    // the headers beside it are the other half of the same job for an instance
    // whose credential is a token rather than a cookie — merged UNDER the
    // caller's, so the notifier's own headers still win. Explicit here for the
    // reason `probeVersion`'s are: a badge that silently stays at zero because
    // the stream authenticated differently from the window is the exact bug
    // the per-instance session was introduced to fix.
    fetchImpl: (url, init) =>
      sess.fetch(url, {
        credentials: "include",
        ...init,

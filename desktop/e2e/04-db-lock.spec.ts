/* A database already claimed by another Calandria reads as a refusal, not a
 * crash.
 *
 * `server.js` catches `DbLockHeldError`, prints who holds it and exits 1.
 * The shell's job is to say the right thing about that: `supervisor.js`'s
 * `spawnChild` classifies an app-sidecar exit(1) whose log names the holder
 * as `dbLockHeld`, and `main.js`'s `onExit` turns that into "Another
 * Calandria instance is already running against this database" instead of
 * "the app crashed (code 1)" plus twenty lines of log tail.
 *
 * Two main-process calls are replaced with recorders, and both replacements
 * serve the assertion:
 *
 *   `dialog.showErrorBox` is modal and synchronous. Under Xvfb there is no
 *   window manager to dismiss it, so leaving it in place would block the
 *   main process forever and this spec would only ever report a timeout.
 *
 *   `app.exit` runs immediately after. Recording it instead of letting it
 *   run lets the spec read the exit code the shell chose and then shut the
 *   instance down through `app.quit()`; otherwise `app.exit(1)` orphans the
 *   pty sidecar, which outlives the run holding a port.
 *
 * The patches are installed right after `electron.launch()` resolves, at
 * Electron process start; the collision cannot happen until the sidecar has
 * booted Node, loaded the app and waited out `CALANDRIA_DB_LOCK_WAIT_MS`.
 * The margin is seconds against a round trip of milliseconds.
 */
...
test('a held database reads as "another Calandria is already running"', async () => {
  ...
  // `lib/db-lock.mjs` mutex server.js takes at boot, on a database nothing
  // else has ever opened.

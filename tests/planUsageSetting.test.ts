...
// ("plan_usage:<agent>", Settings -> Agents). It rides /api/settings rather
// than browser storage so the choice follows the instance to every device, and
// the client reads it as shown-unless-"off" — so the write that HIDES a tracker
// is a value and the one that shows it again is a clear.
describe("PATCH /api/settings — the plan-usage key", () => {
  ...
  it("stores and clears an agent-scoped key for any agent id", async () => {
    ...
    // Showing it again clears the row rather than storing "on": absent is the

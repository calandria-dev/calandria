 ...
 *
 * The POSIX half must create at 0600, re-chmod an existing file, and never
 * fail the save over the chmod. The win32 half can't run here: NTFS and
...
// attribute and nothing else, which is why lib/secretFile.ts exists, so
// `statSync(...).mode` can never report 0o600 there.
describe.skipIf(IS_WIN)("writeSecretFile on POSIX", () => {
  ...
  it("narrows a file that already existed with a wider mode", () => {
    ...
    // repairs a key with a wider mode, whether from a bad umask or otherwise.
...
describe("writeSecretFile on win32", () => {
  ...
  it("sets an owner-only ACL instead of trusting the mode", () => {
    ...
    // re-saving also repairs a file previously left world-readable.
  ...
  it("names the caller's escape hatch in the error", () => {
    // The route surfaces this string to the wizard, so it must say what to do
  ...
  it("keeps the file and warns under fatal: false", () => {
    // The VAPID keypair's policy (lib/push/vapid.ts): it is minted with no
    // user in the loop, so throwing here would take push notifications out
    // entirely on a filesystem with no ACLs. The blast radius is smaller than
    // an API key's: at most forged pushes to this instance's own subscribers,
    // and unlike the API keys it is not readable through /api/settings either.
...
describe("windowsAclCommand", () => {
  it("pins icacls to %SystemRoot%\\System32 rather than PATH order", () => {
    // A PATH-resolved icacls is one writable directory away from being
    // someone else's program that exits 0; this call is what prevents that.
...
describe("secretFileOwner", () => {
  // The account comes from os.userInfo(), authoritative on both platforms; the
  // env is only a fallback for it. Only the DOMAIN half is env-derived.
...
describe("the secrets on the volume go through it", () => {
  ...
  it("writes both key files owner-only", () => {
    ...
    // elsewhere), since it is the production path. On both platforms the file
    // must exist afterwards: a failed lock-down deletes it and throws instead
    // of leaving a key at permissions that could not be set. The mode is only
    // readable on POSIX (see the skip above).
  ...
  it("writes the generated VAPID keypair owner-only too", () => {
    ...
    // here; the win32 ACL it also gets is pinned structurally above.

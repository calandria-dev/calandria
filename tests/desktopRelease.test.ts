...
describe("no lane publishes by accident", () => {
  ...
  // The release lane passes `--publish` as an expression
  // (`always` on a tag, `never` otherwise), so it is asserted on separately
  // rather than being expected to read literally "never".

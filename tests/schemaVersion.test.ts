...
//
// Pins the case where an operator rolls an image tag BACK after an upgrade
// and the older build opens a database a newer one already migrated: there is
// no error, only a build running against a schema it doesn't understand. The
// cases below assert that the refusal happens, that it happens BEFORE
// anything is written, and that the ordinary directions (fresh install, older
// database, same version) are untouched.
...
describe("schema version stamp", () => {
  it("stamps the current version at the end of migrate()", () => {
    ...
    // The bare shape migrate() runs over: everything it touches is
  ...
  it("moves an unstamped (pre-gate) database forward rather than refusing it", () => {
    ...
    const db = openScratchDb(dir);
    init(db);
    // A database with no stamp reads back as 0.
...
describe("boot gate", () => {
  ...
  it("says what to do about it: the newer tag, or the pre-upgrade backup", () => {
    ...
    // Both numbers, so "newer" is checkable, not just asserted...
    expect(message).toContain(`${SCHEMA_VERSION + 4}`);
    expect(message).toContain(`${SCHEMA_VERSION}`);
    // ...and both exits: the refusal is all the operator gets, since the app
  ...
  it("refuses BEFORE writing anything, so the older build leaves no trace", () => {
    ...
    // Not even the journal mode: the gate runs ahead of every pragma that init sets.
  ...
  it("lets an older-or-equal stamp through, in both directions", () => {
    ...
    // Older: the ordinary upgrade, where additive migrations bring it forward.
    db.pragma("user_version = 0");
    expect(() => assertSchemaVersionSupported(db)).not.toThrow();

...
// server.js's SIGTERM/SIGINT handler pings via POST /api/instance/drain
// before exiting. It reuses the Stop-button mechanics (abortTurn and the
// finally's DENIED_INTERRUPTED settlement), waits for the real completion
// signal (turn_end) since hasTurn() reports done immediately, and never
// hangs the shutdown past its own bound.
//
// Same seam as tests/permissionGate.test.ts and tests/turnRace.test.ts: only
// the SDK-driving module is swapped, so the registry, runner, store and
...
describe("drainActiveTurns (graceful shutdown)", () => {
  ...
  it("aborts a live turn, waits for its finally, and settles an open permission card exactly like a Stop", async () => {
    ...
    // Parks on a permission prompt and never decides. The event queue closes
    // with the SDK stream once aborted; only the runner's finally settles the
    // card.
  ...
  it("drains every live turn across tasks in one pass", async () => {
    ...
    runTurnMock.mockImplementation(async function* (_t: unknown, _p: unknown, _u: string, ac?: AbortController) {
      const i = call++;
      // Registered before the first yield, matching where a real driver wires
      // up its abort handling at the top of runTurn. Registering later would
      // race a synchronous abortTurn() called while this generator is still
      // suspended on its first yield.
  ...
  it("gives up at the bound instead of hanging when a turn won't unwind", async () => {
    ...
    // Ignores the abort signal entirely, modeling a driver stuck on an

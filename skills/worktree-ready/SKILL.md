...
- It contains **tracked files at one commit, and nothing else**. No `.env`, no
  ...
  tool cut the worktree before assuming it works the same way.
- **No bootstrap runs.** Calandria has no per-worktree setup hook: the worktree
  is cut and the agent's first turn starts in it. A human or the agent has to
  do whatever the repo needs done first, so make it one command and name that
  command somewhere the agent reads.
- It lives **outside the repo** (`~/.calandria/worktrees/<task-id>`), on a
  branch `calandria/<task-id>` cut from a pinned base SHA. Living outside the
  repo avoids a separate problem: a worktree nested inside a package-manager
  workspace root corrupts the parent's `node_modules`.
...
The interesting findings are the ones nobody remembers: a `.env.local` that
has been on disk for two years, a port literal in a test helper. Don't stop at
the probe's output. It can only see patterns, so confirm each finding in the
file before proposing a change, and read the repo's own setup docs for
anything it can't detect structurally (a service that must be running, an
account that must exist).
...
**Why it breaks:** on the first turn, loudest, and often as a confusing error
rather than "file not found". An empty `DATABASE_URL` reads as a connection
bug.

**Fix, in order of preference:**

1. Make the file unnecessary: give real defaults in code for everything that
   isn't a secret, so a bare checkout runs.
...
tooling does. Reach for it last: it re-shares exactly the state (one database,
one port, one API key) that this whole exercise is meant to separate.
...
port bindings, and anything a test binds. Two tasks running the app at once is
the normal case, not an edge case.

**Why it breaks:** it breaks badly with agents specifically. `EADDRINUSE` reads
like a bug in the code, and an agent will cheerfully spend a turn editing
application logic over it.

**Fix:**

- Make every port env-driven with a documented default: `PORT ?? 3000`, and
  document the default in `.env.example`.
- **Tests bind port 0** (ephemeral) and read back the assigned port. A fixed
  test port collides the moment two suites run at once, and running tests in
  parallel tasks is the point of this setup.
- Under Calandria specifically, managed services get a `PORT` injected, but an
  agent running `npm run dev` itself in its worktree gets nothing. Honoring
  `PORT` is what lets the agent pick a free one; `expose_service` is how it
...
- Derive the database name, data directory or socket path from the worktree, or
...
worktree, so the path doesn't resolve. Inside a container it can't resolve at
all. Real instances: Composer defaults the root package version to `1.0.0`;
containerized CI runners fail `actions/checkout`; a tool that cached its
project id at `.git/<name>` computed a different id per worktree and
fragmented its own state.

**Fix:** use `git rev-parse --git-dir` / `--git-common-dir` / `--git-path
<file>` instead of literal `.git/...`. Decide which one you need:
`--git-common-dir` for anything shared by the whole repo (refs, the object
store, a project identity), `--git-dir` for anything private to this worktree.
...
- **LFS** materializes a full copy of every tracked blob per worktree. Nothing
  dedupes it. Name this in the repo's docs as a disk-cost warning.
- **Hooks** live in the shared git dir, so they already apply to every worktree.
  ...
  for correctness rather than convenience, state that.

### 7. One bootstrap command

Everything above converges here. No setup hook runs automatically, so the
repo's answer to "you have a bare checkout, now what" has to be a single
idempotent command, such as `make bootstrap`, `just setup`, `npm run setup`,
or `./scripts/setup.sh`, that installs dependencies, seeds config from
examples, initializes submodules, and does nothing when already done.
...
checkout". A bootstrap command nobody is told about does no good, and this
file is the only per-worktree setup mechanism that exists today.
...
is a legitimate finding. Leaving it out silently is not.
...
- **Don't add a tool.** A worktree manager, a port broker or a devcontainer may
  well be right for the user, but that's their call and it's out of scope for a
  repo audit. Note it as an option, with what it would buy.

## Prove it

The check that actually settles it: run this from the repo, not from a
worktree you're already working in (you can't check out the same branch
twice).
...
- `references/fixes.md`: copy-paste recipes for a bootstrap script skeleton,
  compose namespacing, env-driven ports, ephemeral test ports, and an
